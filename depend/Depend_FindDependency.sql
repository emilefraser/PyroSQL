CREATE PROCEDURE meta.Depend_FindDependency
AS
BEGIN
...
DECLARE @rows INT;
 ...
 UPDATE
	 #t1
 SET
	 object_id	 = tt.user_type_id
   , relative_id = tt.user_type_id
 FROM
	 sys.table_types AS tt
 WHERE
	 tt.type_table_object_id = #t1.object_id
...
WHILE @rows > 0
 ...
 SELECT DISTINCT
	 t.object_id, t.object_name, t.object_schema, t.object_type
 FROM
	 #t1 AS t
 WHERE
	 @iter_no = t.rank
	 ...
	 SELECT
		 ...
		 dep.referencing_class
			 WHEN 1
				 THEN (
						 SELECT
							 CASE WHEN obj.type = 'U' THEN @u WHEN obj.type = 'V' THEN @v WHEN obj.type = 'TR' THEN @tr WHEN obj.type IN ('P', 'RF', 'PC') THEN @sp WHEN obj.type IN ('AF') THEN @uda WHEN obj.type IN ('TF', 'FN', 'IF', 'FS', 'FT') THEN @udf WHEN obj.type = 'D' THEN @def WHEN obj.type = 'SN' THEN @synonym WHEN obj.type = 'SO' THEN @sequence ELSE @obj END
						 FROM
							 sys.objects AS obj
						 WHERE
			 ...
			 WHEN 6
				 THEN (
						 SELECT
							 CASE WHEN (tp.is_assembly_type = 1) THEN @udt WHEN (tp.is_table_type = 1) THEN @udtt ELSE @uddt END
						 FROM
							 sys.types AS tp
						 WHERE
	   ...
	   , @iter_no + 1
	 FROM
		 sys.dm_sql_referencing_entities(@fullname, @objecttype) AS dep;
 ...
 WHERE
	 o.object_id = #t2.object_id
	 AND (#t2.object_type = @obj OR o.parent_object_id != 0)
 ...
 DECLARE
	 @dbname SYSNAME;
 DECLARE
	 @quote_quoted_dbname SYSNAME;
 DECLARE
	 @bracket_quoted_dbname SYSNAME;
 ...
 SET
   ...
   , object_type = @sp
 FROM
	 master.sys.objects AS o
 JOIN
	 master.sys.schemas AS sch
	 ON sch.schema_id = o.schema_id
 WHERE
	 o.name = #t2.object_name COLLATE database_default
...
WHILE ( @@FETCH_STATUS <> -1 )
 ...
 (
	 SELECT
		 CONVERT(SYSNAME, DATABASEPROPERTYEX(@dbname, 'Collation'))
 );
 SET @query =
...
DEALLOCATE db_cursor;
 ...
 --final select
 SELECT
	 ISNULL(t.object_id, 0)				  AS [object_id]
   , t.object_name
