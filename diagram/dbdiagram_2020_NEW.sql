...
CREATE OR ALTER PROCEDURE dbo.Generate_DbDiagram
...
,	@Database_ToDiagram				SYSNAME			= 'MrCatalog'
,	@Schema_ToDiagram				SYSNAME			= 'DC'
...
SET @sql_statement = '
SELECT
	obj.Type_Desc, QUOTENAME(obj.name), ''' + QUOTENAME(@Database_ToDiagram) + '.' + QUOTENAME(@Schema_ToDiagram) + '''
FROM 
	[MrCatalog].sys.objects AS obj
INNER JOIN 
	[MrCatalog].sys.schemas AS sch
...
select 
    ...
    col.precision,
    col.is_nullable,
	QUOTENAME(@Database_ToDiagram) + '.' + QUOTENAME(@Schema_ToDiagram) + '.' + QUOTENAME(tab.name)
from [MrCatalog].sys.tables as tab
inner join [MrCatalog].sys.schemas AS sch
On sch.Schema_ID = tab.Schema_ID
    inner join [MrCatalog].sys.columns as col
        on tab.object_id = col.object_id
    left join [MrCatalog].sys.types as t
    on col.user_type_id = t.user_type_id
where tab.name = 'Field' -- enter table name here
and sch.name = 'DC'
