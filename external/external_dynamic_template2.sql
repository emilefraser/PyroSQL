...
-- DROP AND CREATE DATABASE SCOPED ScopedCredentialName
SET @sql_statement = 
    'IF EXISTS ( 
        SELECT 1 
        FROM sys.database_scoped_credential
        WHERE name = ''' + @ScopedCredentialName + '''
     )
     DROP DATABASE SCOPED CREDENTIAL ' + @ScopedCredentialName + @sql_crlf

SET @sql_statement += 
    'CREATE DATABASE SCOPED CREDENTIAL ' + @ScopedCredentialName + '
     WITH IDENTITY = ''' + @ScopedIdentity + '''
     SECRET = ''' + @ScopedSecret + ''';'
