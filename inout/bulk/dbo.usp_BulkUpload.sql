IF OBJECT_ID(N'dbo.usp_BulkUpload', N'P') IS NULL
BEGIN
	EXEC (N'CREATE PROCEDURE dbo.usp_BulkUpload AS SELECT 1');
END
GO
...
BEGIN TRY
	DECLARE 
		   @databaseRecoveryModeCurrent    NVARCHAR(15);
	DECLARE 
		   @schemaTableName    NVARCHAR(600) = QUOTENAME(@schemaName) + '.' + QUOTENAME(@tableName);
	DECLARE 
		   @tsqlCommand    NVARCHAR(MAX) = '';
	DECLARE 
		   @ParamDefinitionIndentity    NVARCHAR(500) = N'@identityColumnNameIN NVARCHAR(200), @ColumnsOUT VARCHAR(MAX) OUTPUT';
	DECLARE 
		   @tableFullName    NVARCHAR(600) = CASE
												 WHEN
		@databaseName <> ''
													 THEN QUOTENAME(@databaseName) + '.'
												 ELSE ''
											 END + @schemaTableName;
	DECLARE 
		   @#tableName    NVARCHAR(600) = QUOTENAME('#' + @tableName);
	DECLARE 
		   @OBJECT_ID    INTEGER = OBJECT_ID(@tableFullName);
	DECLARE 
		   @Columns    NVARCHAR(MAX) = N'';
	DECLARE 
		   @filePath    NVARCHAR(MAX) = @path + CASE
													WHEN
		@fileName = ''
														THEN @tableFullName
													ELSE @fileName
	...
	IF @FORMATFILE NOT IN('', 'xml', 'fmt')
	BEGIN
		THROW 50004, 'Allowed values for parameter is xml, fmt or blank value', 1;
	END
	IF
		@FORMATFILE = ''
		AND
		@skipTempDB = 1
	BEGIN
		THROW 50005, 'If @skipTempDB = 1 then @FORMATFILE must be in xml or fmt', 1;
	END
	...
	IF
		RIGHT(@path, 1) <> '\'
	BEGIN
		THROW 50001, 'Please add a slash (\) at the end of a variable @path!!!', 1;
	END

	IF
		LEFT(@databaseName, 1) = N'['
		OR
		LEFT(@tableName, 1) = N'['
		OR
		LEFT(@schemaName, 1) = N'['
	...
	IF
		@debug = 1
	BEGIN
		PRINT ISNULL(N'@filePath = {' + @filePath + N'}', N'@filePath = Null');
	END

	IF
		@ERRORFILE = ''
	BEGIN
		SET @ERRORFILE = @path + @tableFullName + N'_error_' + REPLACE(CONVERT(NCHAR(23), GETDATE(), 126), ':', '_') + N'.txt';
	END
	...
	BEGIN
		SELECT 
			@databaseRecoveryModeCurrent = [recovery_model_desc]
		FROM 
			sys.databases
		WHERE
			[name] = @databaseName;
		IF
			@debug = 1
		BEGIN
			PRINT ISNULL('@databaseRecoveryModeCurrent = {' + @databaseRecoveryModeCurrent + '}', '@databaseRecoveryModeCurrent = Null');
		END
		...
		IF
			@databaseRecoveryMode IN('FULL', 'BULK_LOGGED', 'SIMPLE')
			AND
			@debug = 0
			AND
			@databaseRecoveryModeCurrent <> @databaseRecoveryMode
		BEGIN
	...
	IF
		...
		@identityColumnName = ''
	BEGIN
		SET @identityColumnName = @tableName + 'ID';
	END
	IF
		@debug = 1
	BEGIN
		PRINT ISNULL('@identityColumnName = {' + @identityColumnName + '}', '@identityColumnName = Null');
	END

	SET @tsqlCommand = N'USE __DBName__;
...
';
	...
	SET @tsqlCommand = REPLACE(@tsqlCommand, '__columTS__',
											 CASE
												 WHEN
		@columnTypeSort = 1
													 THEN N'ORDER BY Name;'
												 ELSE N'ORDER BY column_id;'
											 END);

	IF
		@debug = 1
	BEGIN
		PRINT ISNULL('@OBJECT_ID = {' + CAST(@OBJECT_ID AS NVARCHAR) + '}', '@OBJECT_ID = Null');
	END
	IF
		@debug = 1
	BEGIN
	...
	SET @tsqlCommand = 'IF OBJECT_ID(''tempdb..__#tableName__'') IS NOT NULL DROP TABLE __#tableName__;
...
';
	...
	IF
		...
		@columnTypeSort = 0
	BEGIN
		SET @tsqlCommand = 'BULK INSERT __tableFullName__
FROM ''__filePath__''
WITH (
      __BATCHSIZE__
      FIELDTERMINATOR = ''__FIELDTERMINATOR__''
...
'
	END
	SET @tsqlCommand = REPLACE(@tsqlCommand, '__FORMATFILE__',
											 CASE
												 WHEN
		@FORMATFILE = 'xml'
													 THEN ',FORMATFILE = ''' + @path + @schemaTableName + '.xml'''
												 WHEN
		@FORMATFILE = 'fmt'
													 THEN ',FORMATFILE = ''' + @path + @schemaTableName + '.fmt'''
												 ELSE ''
											 END);
	...
	SET @tsqlCommand = REPLACE(@tsqlCommand, '__BATCHSIZE__',
											 CASE
												 WHEN
		@BATCHSIZE > 0
													 THEN 'BATCHSIZE = ' + CAST(@BATCHSIZE AS NVARCHAR) + ','
												 ELSE ''
											 END);
	...
	SET @tsqlCommand = REPLACE(@tsqlCommand, '__CHECK_CONSTRAINTS__',
											 CASE
												 WHEN
		@CHECK_CONSTRAINTS = 1
													 THEN ',CHECK_CONSTRAINTS'
												 ELSE ''
											 END);
	SET @tsqlCommand = REPLACE(@tsqlCommand, '__DATAFILETYPE__', @DATAFILETYPE);
	SET @tsqlCommand = REPLACE(@tsqlCommand, '__KEEPIDENTITY__',
											 CASE
												 WHEN
		@useIdentity = 1
													 THEN ',KEEPIDENTITY'
	...
	SET @tsqlCommand = REPLACE(@tsqlCommand, '__FIRE_TRIGGERS__',
											 CASE
												 WHEN
		@FIRE_TRIGGERS = 1
													 THEN ',FIRE_TRIGGERS'
												 ELSE ''
											 END);
	SET @tsqlCommand = REPLACE(@tsqlCommand, '__KEEPNULLS__',
											 CASE
												 WHEN
		@KEEPNULLS = 1
													 THEN ',KEEPNULLS'
												 ELSE ''
											 END);
	SET @tsqlCommand = REPLACE(@tsqlCommand, '__KILOBYTES_PER_BATCH__',
											 CASE
												 WHEN
		@KILOBYTES_PER_BATCH > 0
													 THEN ',KILOBYTES_PER_BATCH = ' + CAST(@KILOBYTES_PER_BATCH AS NVARCHAR)
												 ELSE ''
											 END);
	SET @tsqlCommand = REPLACE(@tsqlCommand, '__LASTROW__',
											 CASE
												 WHEN
		@LASTROW > 0
													 THEN ',LASTROW = ' + CAST(@LASTROW AS NVARCHAR)
												 ELSE ''
											 END);
	SET @tsqlCommand = REPLACE(@tsqlCommand, '__TABLOCK__',
											 CASE
												 WHEN
		@TABLOCK = 1
													 THEN ',TABLOCK'
												 ELSE ''
											 END);
	SET @tsqlCommand = REPLACE(@tsqlCommand, '__ROWS_PER_BATCH__',
											 CASE
												 WHEN
		@ROWS_PER_BATCH > 0
													 THEN ',ROWS_PER_BATCH = ' + CAST(@ROWS_PER_BATCH AS NVARCHAR)
												 ELSE ''
											 END);
	SET @tsqlCommand = REPLACE(@tsqlCommand, '__MAXERRORS__',
											 CASE
												 WHEN
		@MAXERRORS > 0
													 THEN ',MAXERRORS = ' + CAST(@MAXERRORS AS NVARCHAR)
												 ELSE ''
											 END);
	SET @tsqlCommand = REPLACE(@tsqlCommand, '__ERRORFILE__', @ERRORFILE);
	SET @tsqlCommand = REPLACE(@tsqlCommand, '__useIdentityON__',
											 CASE
												 WHEN
		@useIdentity = 1
													 THEN 'SET IDENTITY_INSERT ' + @tableFullName + ' ON;'
												 ELSE ''
											 END);
	SET @tsqlCommand = REPLACE(@tsqlCommand, '__rowOrder__',
											 CASE
												 WHEN
		@rowOrderByColumn <> ''
													 THEN 'ORDER BY ' + @rowOrderByColumn
												 ELSE ''
											 END);
	SET @tsqlCommand = REPLACE(@tsqlCommand, '__useIdentityOFF__',
											 CASE
												 WHEN
		@useIdentity = 1
													 THEN 'SET IDENTITY_INSERT ' + @tableFullName + ' OFF;'
												 ELSE ''
											 END);
	...
	IF
		@debug = 1
	BEGIN
		PRINT ISNULL(CAST('@tsqlCommand = {' + @crlf + @tsqlCommand + @crlf + '}' AS NTEXT), '@tsqlCommand = {Null}' + @crlf + '--End Deubg*********/')
	END
		ELSE
	BEGIN
		EXECUTE sp_executesql 
...
BEGIN CATCH
	...
	PRINT 'Error: ' + CONVERT(VARCHAR(50), ERROR_NUMBER()) + ', Severity: ' + CONVERT(VARCHAR(5), ERROR_SEVERITY()) + ', State: ' + CONVERT(VARCHAR(5), ERROR_STATE()) + ', Procedure: ' + ISNULL(ERROR_PROCEDURE(), '-') + ', Line: ' + CONVERT(VARCHAR(5), ERROR_LINE()) + ', User name: ' + CONVERT(SYSNAME, CURRENT_USER);
	PRINT ERROR_MESSAGE();
END CATCH
END;
GO