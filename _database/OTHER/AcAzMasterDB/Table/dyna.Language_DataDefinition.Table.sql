...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dyna].[Language_DataDefinition]') AND type in (N'U'))
BEGIN
CREATE TABLE [dyna].[Language_DataDefinition](
...
WITH
(
SYSTEM_VERSIONING = ON ( HISTORY_TABLE = [dyna].[Language_DataDefinition_History] )
)
END
GO
