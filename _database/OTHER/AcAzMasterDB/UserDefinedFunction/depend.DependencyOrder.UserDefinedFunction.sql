...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[depend].[DependencyOrder]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
BEGIN
execute dbo.sp_executesql @statement = N'CREATE FUNCTION [depend].[DependencyOrder] ()
/* 
summary:   >
...
Revisions:
 - Author: Phil Factor
   ...
   date: 3rd Sept 2015
 example:
     - code: Select * from depend.DependencyOrder() order by theorder desc
returns:   >
