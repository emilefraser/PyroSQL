...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[Generator].[GetFullNameParts]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [Generator].[GetFullNameParts] AS' 
END
GO
ALTER   PROCEDURE [Generator].[GetFullNameParts]
	@Count BIGINT
AS
/******************************************************************************
