...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[depend].[DependAllObjects]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [depend].[DependAllObjects] AS' 
END
GO
/*
	EXEC depend.DependAllObjects 
		@objectName		= 'azure.ArmSchema'
	,	@objectClass	= N'OBJECT'
	,	@debug			= 1
*/
ALTER   PROCEDURE [depend].[DependAllObjects] (
