...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_GetDDLa]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[sp_GetDDLa] AS' 
END
GO
--#################################################################################################
...
--#############################################################################
ALTER PROCEDURE [dbo].[sp_GetDDLa]
  @TBL                VARCHAR(255)
AS
BEGIN
  SET NOCOUNT ON;
...
GO
GRANT EXECUTE ON [dbo].[sp_GetDDLa] TO [public] AS [dbo]
GO
