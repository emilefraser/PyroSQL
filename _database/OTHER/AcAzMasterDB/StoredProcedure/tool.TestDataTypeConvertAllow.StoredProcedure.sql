...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[tool].[TestDataTypeConvertAllow]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [tool].[TestDataTypeConvertAllow] AS' 
END
GO
/*
...
ALTER    PROCEDURE [tool].[TestDataTypeConvertAllow]
AS
BEGIN
