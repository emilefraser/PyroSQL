...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[GenerateAllDdl]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[GenerateAllDdl] AS' 
END
GO
-- EXEC dbo.GenerateAllDdl(@WithData int = 0)
ALTER   PROCEDURE [dbo].[GenerateAllDdl](@WithData int = 0)
AS
BEGIN
  SET NOCOUNT ON
