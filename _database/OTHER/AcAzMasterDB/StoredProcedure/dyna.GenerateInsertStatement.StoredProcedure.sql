...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dyna].[GenerateInsertStatement]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dyna].[GenerateInsertStatement] AS' 
END
GO
/*
{{META>}}
	...
	{Usage>}		
					EXEC dyna.GenerateInsertStatement
	{<Usage}
{{<META}}
*/
ALTER   PROCEDURE [dyna].[GenerateInsertStatement]
AS
BEGIN
DECLARE 
