...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dba].[TransferPrimaryUniqueKeysPerSchema]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dba].[TransferPrimaryUniqueKeysPerSchema] AS' 
END
GO
...
ALTER      PROCEDURE [dba].[TransferPrimaryUniqueKeysPerSchema]
