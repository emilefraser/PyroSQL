...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dss].[CheckAndDeleteUnusedDatabase]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dss].[CheckAndDeleteUnusedDatabase] AS' 
END
GO
ALTER PROCEDURE [dss].[CheckAndDeleteUnusedDatabase]
