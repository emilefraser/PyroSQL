...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[inout].[CreateExternalDataSource]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [inout].[CreateExternalDataSource] AS' 
END
GO
...
-- Date			: 2021-01-02
-- Description	: DROPS ANY MASTER KEY IF EXISTS AND CREATES NEW MASTER KEY

-- TODO			: Specify Azure Versions where Master Key cannot be dropped

-- Test			EXEC [inout].[CreateExternalDataSource] 
									@Password = '105022_Alpha'

*/
ALTER   PROCEDURE [inout].[CreateExternalDataSource]
...
BEGIN
	...
	SET @sql_statement = '
		...
		-- CREATES A MASTER KEY WITH THE SPECIFIED PASSWORD
		IF NOT EXISTS  (
	...
	SET @sql_parameter = NULL --'@Password NVARCHAR(128)'
