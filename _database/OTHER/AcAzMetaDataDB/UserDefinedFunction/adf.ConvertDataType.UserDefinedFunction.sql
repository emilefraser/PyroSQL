...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[adf].[ConvertDataType]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
BEGIN
execute dbo.sp_executesql @statement = N'/*
	...
	Test1: SELECT [adf].[ConvertDataType]( ''DEC'', ''ABAP'', ''000020'', ''000020'', ''X'', ''MSSQL'')

*/
CREATE   FUNCTION [adf].[ConvertDataType] (
...
--,	@TagetTechnologyTypeCode	SYSNAME =  ''MSSQL''
	...
	FROM 									  
		[adf].[vw_DataTypeConvert] AS vdtc
	WHERE 
