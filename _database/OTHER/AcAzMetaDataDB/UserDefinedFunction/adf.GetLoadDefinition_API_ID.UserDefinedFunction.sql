...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[adf].[GetLoadDefinition_API_ID]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
BEGIN
...
/*
	...
	Date: 2020-11-01
	Description: Gets the Load Definition for ADF Incremental Load from an API using incremental ID
	SELECT * FROM adf.LoadConfig WHERE SourceEntityName = ''EKKO''
	Test1: SELECT [adf].[GetLoadDefinition_INCR_ID](180, ''SOURCE'')
	Test2: SELECT [adf].[GetLoadDefinition_INCR_ID](5, ''TARGET'')

	TODO: Make the Waterfall field dynamic

	
*/
CREATE       FUNCTION [adf].[GetLoadDefinition_API_ID] (
	@LoadConfigID			INT		
