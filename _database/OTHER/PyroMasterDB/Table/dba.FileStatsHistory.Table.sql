...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dba].[FileStatsHistory]') AND type in (N'U'))
BEGIN
CREATE TABLE [dba].[FileStatsHistory](
...
END
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dba].[DF_FileStatsHistory_DateStamp]') AND type = 'D')
BEGIN
ALTER TABLE [dba].[FileStatsHistory] ADD  CONSTRAINT [DF_FileStatsHistory_DateStamp]  DEFAULT (getdate()) FOR [FileStatsDateStamp]
END
GO
