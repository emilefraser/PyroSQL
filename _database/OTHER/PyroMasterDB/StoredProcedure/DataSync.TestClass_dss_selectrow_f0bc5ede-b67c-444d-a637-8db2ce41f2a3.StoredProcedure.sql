...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[DataSync].[TestClass_dss_selectrow_f0bc5ede-b67c-444d-a637-8db2ce41f2a3]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [DataSync].[TestClass_dss_selectrow_f0bc5ede-b67c-444d-a637-8db2ce41f2a3] AS' 
END
GO
ALTER PROCEDURE [DataSync].[TestClass_dss_selectrow_f0bc5ede-b67c-444d-a637-8db2ce41f2a3]
	@P_1 SmallInt,
...
THEN [side].[TestClassId]
ELSE [base].[TestClassId] END) 
as [TestClassId], 
[base].[TestClassCode], [base].[TestClassName], [base].[CreatedDT], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [test].[TestClass] WHERE [TestClassId] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[TestClass_dss_tracking] WHERE [TestClassId] = @P_1) [side] ON [base].[TestClassId] = [side].[TestClassId]
END
GO
