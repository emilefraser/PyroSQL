...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[string].[GetString_or_binary_data_truncated]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [string].[GetString_or_binary_data_truncated] AS' 
END
GO


ALTER     PROCEDURE [string].[GetString_or_binary_data_truncated]
@String VARCHAR(MAX)
AS
