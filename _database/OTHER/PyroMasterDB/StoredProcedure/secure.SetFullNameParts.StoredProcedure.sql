...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[secure].[SetFullNameParts]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [secure].[SetFullNameParts] AS' 
END
GO
ALTER     PROCEDURE [secure].[SetFullNameParts]
...
INSERT INTO #FakeData EXEC generate.GetFullNameParts @RowCount;
