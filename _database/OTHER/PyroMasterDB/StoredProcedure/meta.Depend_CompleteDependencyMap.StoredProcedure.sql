...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[meta].[Depend_CompleteDependencyMap]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [meta].[Depend_CompleteDependencyMap] AS' 
END
GO
...
*/
ALTER   PROCEDURE [meta].[Depend_CompleteDependencyMap]
AS
BEGIN
