...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[create_obj]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[create_obj] AS' 
END
GO
...
select * from dbo.obj_ext
*/ 
ALTER   PROCEDURE [dbo].[create_obj] @obj_name as varchar(255), @obj_type as varchar(255) , @server_type_id as int, @is_linked_server as bit=0 , @transfer_id as int = 0 as 
begin 
	set nocount on 
	declare 
		@sql as varchar(max) = ''
