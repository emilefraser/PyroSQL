...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[meta].[ExportSchema]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [meta].[ExportSchema] AS' 
END
GO
--#################################################################################################
...
ALTER PROCEDURE [meta].[ExportSchema] @TargetSchemaName VARCHAR(128)
AS
BEGIN
SET NOCOUNT ON;
