...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[automate].[DropAllObjectsInSchema]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [automate].[DropAllObjectsInSchema] AS' 
END
GO
/*
	EXEC [automate].[DropAllObjectsInSchema] 
*/
ALTER PROCEDURE [automate].[DropAllObjectsInSchema] 
	@schemaname SYSNAME
AS
BEGIN
