...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[construct].[sp_GetTableDef]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [construct].[sp_GetTableDef] AS' 
END
GO
...
-- DROP PROCEDURE [construct].[sp_GetTableDef]
--#############################################################################
...
--#############################################################################
ALTER PROCEDURE [construct].[sp_GetTableDef]
  @TBL                VARCHAR(255)
AS
BEGIN
  SET NOCOUNT ON
