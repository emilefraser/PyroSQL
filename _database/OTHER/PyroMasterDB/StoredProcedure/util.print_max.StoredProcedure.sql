...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[util].[print_max]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [util].[print_max] AS' 
END
GO
/* https://weblogs.asp.net/bdill/sql-server-print-max
exec util.print_max 'atetew tewtew'
*/
ALTER  PROCEDURE [util].[print_max](@iInput NVARCHAR(MAX) ) 
AS
BEGIN
    IF @iInput IS NULL
    RETURN;
