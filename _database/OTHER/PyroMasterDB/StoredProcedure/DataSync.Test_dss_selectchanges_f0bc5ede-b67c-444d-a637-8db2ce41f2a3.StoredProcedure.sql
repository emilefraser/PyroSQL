...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[DataSync].[Test_dss_selectchanges_f0bc5ede-b67c-444d-a637-8db2ce41f2a3]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [DataSync].[Test_dss_selectchanges_f0bc5ede-b67c-444d-a637-8db2ce41f2a3] AS' 
END
GO
ALTER PROCEDURE [DataSync].[Test_dss_selectchanges_f0bc5ede-b67c-444d-a637-8db2ce41f2a3]
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM [test].[Test] [base] FULL OUTER JOIN [DataSync].[Test_dss_tracking] [side] ON [base].[TestId] = [side].[TestId]
 WHERE 
