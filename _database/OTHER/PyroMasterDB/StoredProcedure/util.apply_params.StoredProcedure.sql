...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[util].[apply_params]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [util].[apply_params] AS' 
END
GO
...
print @sql 
*/
ALTER   PROCEDURE [util].[apply_params]
