...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[automate].[sp_foreachdb]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [automate].[sp_foreachdb] AS' 
END
GO

ALTER PROCEDURE [automate].[sp_foreachdb]
