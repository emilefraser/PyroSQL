...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[meta].[GetTableDefinition]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [meta].[GetTableDefinition] AS' 
END
GO
...
-- V304  testing whether vbCrLf is better than just CHAR(13), some formatting cleanup with  statements
...
-- V306  fixes the computed columns definition that t broken/removed somehow in V300
...
-- V312  t rid of all upper casing, and allowing all scripts to generate the exact object names in cases of case sensitive databases.
          ...
          ELSE
            BEGIN
              ...
              + 'DROP ' + CASE 
                            ...
                            ELSE ' FUNCTION  '
                          END 
                          + QUOTENAME(@SCHEMANAME) + '.' + QUOTENAME(@TBLNAME) + ' ' + @vbcrlf + '' + @vbcrlf
              + def.definition 
              FROM sys.objects OBJS 
...
--##############################################################################
  ...
  SELECT
    ...
    + ISNULL(
             ...
             + MODS.definition  + @vbCrLf + '' +  @vbCrLf
             + 'EXEC sp_binderule  ' + quotename(OBJS.[name]) + ', ''' + quotename(OBJECT_NAME(COLS.[object_id])) + '.' + quotename(COLS.[name]) + '''' + @vbCrLf + '' ,'')
  FROM sys.columns COLS 
...
--##############################################################################
  ...
  IF @TRIGGERSTATEMENT <> ''
    SET @TRIGGERSTATEMENT = @vbCrLf + '' + @vbCrLf + @TRIGGERSTATEMENT
--##############################################################################
...
--##############################################################################
 ...
 --SELECT objtype, objname, name, value
  ...
  IF @EXTENDEDPROPERTIES <> ''
    SET @EXTENDEDPROPERTIES = @vbCrLf + '' + @vbCrLf + @EXTENDEDPROPERTIES
--##############################################################################
