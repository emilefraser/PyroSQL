...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[DataSync].[Test_dss_insertmetadata]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [DataSync].[Test_dss_insertmetadata] AS' 
END
GO
ALTER PROCEDURE [DataSync].[Test_dss_insertmetadata]
	@P_1 Int,
