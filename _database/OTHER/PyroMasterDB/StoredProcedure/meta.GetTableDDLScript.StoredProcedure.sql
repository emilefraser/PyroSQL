...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[meta].[GetTableDDLScript]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [meta].[GetTableDDLScript] AS' 
END
GO
/*
---------------------------------------------------------------------------
...
---------------------------------------------------------------------------
*/
ALTER PROCEDURE [meta].[GetTableDDLScript]
(
