...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[generate].[GetFullNameParts]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [generate].[GetFullNameParts] AS' 
END
GO
ALTER     PROCEDURE [generate].[GetFullNameParts]
	@Count BIGINT
AS
/******************************************************************************
* Name     : generate.GetFullNameParts
...
#first, last names.
nm <- generate::r_full_names(total_people)
