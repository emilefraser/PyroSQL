...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dba].[sp_Update_StorageStats_DatabaseFile]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dba].[sp_Update_StorageStats_DatabaseFile] AS' 
END
GO
...
*/
ALTER     PROCEDURE [dba].[sp_Update_StorageStats_DatabaseFile]
			@BatchID INT
AS
BEGIN
