...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[inout].[ImportExternalDatasetWithOpenRowset_FULL]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [inout].[ImportExternalDatasetWithOpenRowset_FULL] AS' 
END
GO
...
-- Test			
	EXEC [inout].[ImportExternalDatasetWithOpenRowset_FULL]
					...
					,	@IsTruncateAndReload			= 1

	SELECT * FROM mssql.ObjectType

*/
ALTER       PROCEDURE [inout].[ImportExternalDatasetWithOpenRowset_FULL]
...
,	@FieldDelimeter					NVARCHAR(5)		= ','
,	@TargetSchemaName				SYSNAME
,	@TargetTableName				SYSNAME
,	@IsTruncateAndReload			BIT				= 0
AS
BEGIN
	...
	,	@ExternalDataSetValue			NVARCHAR(MAX)
	...
	BEGIN
		BEGIN TRY
			...
			-- TRUNCATE current Target table if specifier set
			IF (@IsTruncateAndReload = 1)
			BEGIN
				SET @sql_statement = 'TRUNCATE TABLE ' + QUOTENAME(@TargetSchemaName) + '.' + QUOTENAME(@TargetTableName)
