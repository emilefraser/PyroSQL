...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_alter_column]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[sp_alter_column] AS' 
END
GO

ALTER PROCEDURE [dbo].[sp_alter_column]
(
  @schemaname SYSNAME
  ,@tablename SYSNAME
  ,@columnname SYSNAME
