...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[construct].[GenerateCreateTableDDL_Foreign]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [construct].[GenerateCreateTableDDL_Foreign] AS' 
END
GO
ALTER   PROCEDURE [construct].[GenerateCreateTableDDL_Foreign] 
