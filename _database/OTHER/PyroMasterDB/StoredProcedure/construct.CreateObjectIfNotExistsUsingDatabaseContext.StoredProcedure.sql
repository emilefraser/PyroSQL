...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[construct].[CreateObjectIfNotExistsUsingDatabaseContext]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [construct].[CreateObjectIfNotExistsUsingDatabaseContext] AS' 
END
GO
...
/*
	EXEC [construct].[DeployObjectDDLUsingDatabaseContext]
										...
										,	@IsCheckObjectExists	= 1
*/

ALTER    PROCEDURE [construct].[CreateObjectIfNotExistsUsingDatabaseContext]
											@ObjectName					SYSNAME
										,	@ObjectType					SYSNAME
