...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[DataSync].[TestPattern_dss_selectrow_f0bc5ede-b67c-444d-a637-8db2ce41f2a3]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [DataSync].[TestPattern_dss_selectrow_f0bc5ede-b67c-444d-a637-8db2ce41f2a3] AS' 
END
GO
ALTER PROCEDURE [DataSync].[TestPattern_dss_selectrow_f0bc5ede-b67c-444d-a637-8db2ce41f2a3]
	@P_1 Int,
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [test].[TestPattern] WHERE [PatternID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[TestPattern_dss_tracking] WHERE [PatternID] = @P_1) [side] ON [base].[PatternID] = [side].[PatternID]
END
GO
