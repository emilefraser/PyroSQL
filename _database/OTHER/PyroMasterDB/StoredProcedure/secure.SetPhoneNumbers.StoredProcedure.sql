...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[secure].[SetPhoneNumbers]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [secure].[SetPhoneNumbers] AS' 
END
GO
ALTER     PROCEDURE [secure].[SetPhoneNumbers]
...
INSERT INTO #FakeData EXEC generate.GetPhoneNumbers
	@Count = @ValueCount;
