...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[meta].[GetDDLaV2]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [meta].[GetDDLaV2] AS' 
END
GO
...
--Purpose: Script Any Table, Temp Table or Object, but with standardized constraint names
--#################################################################################################

ALTER    PROCEDURE [meta].[GetDDLaV2]
  @TBL                VARCHAR(255)
AS
BEGIN
  SET NOCOUNT ON
...
--##############################################################################
  ...
  --which flips the is_ms_shipped bit in sys.objects
    SELECT @ISSYSTEMOBJECT = ISNULL(is_ms_shipped,0),@PROCNAME = ISNULL(name,'sp_GetDDLaV2') FROM sys.objects WHERE OBJECT_ID = @@PROCID
  IF @ISSYSTEMOBJECT IS NULL 
    SELECT @ISSYSTEMOBJECT = ISNULL(is_ms_shipped,0),@PROCNAME = ISNULL(name,'sp_GetDDLaV2') FROM master.sys.objects WHERE OBJECT_ID = @@PROCID
  IF @ISSYSTEMOBJECT IS NULL 
    SET @ISSYSTEMOBJECT = 0  
  IF @PROCNAME IS NULL
    SET @PROCNAME = 'sp_GetDDLaV2'
...
--  other data type 	IE INT, DATETIME, MONEY, CUSTOM DATA TYPE,...
             ...
             + CASE
                 ...
                 --optional section in case NAMED default cosntraints are needed:
                  ELSE ' CONSTRAINT [DF__' + @TBLNAME + '__' + COLS.name + '] DEFAULT ' + ISNULL(def.[definition] ,'')
  ...
  INSERT INTO @Results
    SELECT
      ...
      CASE 
        ...
        THEN QUOTENAME('PK__' + OBJS.[name] + '__' + REPLACE(REPLACE(REPLACE(REPLACE(ISNULL(Index_Columns.index_columns_key, '---'),' asc',''),' desc',''),'[',''),']',''))
        WHEN IDX.is_unique_constraint = 1
          THEN  QUOTENAME('UQ__' + OBJS.[name] + '__' + REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(ISNULL(Index_Columns.index_columns_key, '---'),' asc',''),' desc',''),', ',''),'[',''),']',''))
        ELSE ISNULL(IDX.[name], '---') 
      END AS index_name,
...
SELECT
  ...
  'CONSTRAINT ' + quotename(object_name(conz.parent_object_id) + REPLACE(REPLACE(REPLACE(ChildCollection.ChildColumns ,',',''),'[','_'),']',''))
  + ' FOREIGN KEY (' 
