...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[meta].[GetDDL_All]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [meta].[GetDDL_All] AS' 
END
GO

-- EXEC dbo.GenerateAllDdl(@WithData int = 0)
ALTER     PROCEDURE [meta].[GetDDL_All](@WithData int = 0)
AS
BEGIN
  SET NOCOUNT ON
  ...
  --our list of objects in metaency order
  INSERT #MyObjectHierarchy (TYPE,ONAME,OOWNER,SEQ)
    EXEC sp_msmetaencies @intrans = 1 
