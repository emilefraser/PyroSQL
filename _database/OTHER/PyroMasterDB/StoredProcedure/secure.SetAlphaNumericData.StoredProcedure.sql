...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[secure].[SetAlphaNumericData]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [secure].[SetAlphaNumericData] AS' 
END
GO
ALTER     PROCEDURE [secure].[SetAlphaNumericData]
...
INSERT INTO #FakeData EXEC generate.GetAlphaNumericData @AlphaNumericData;
