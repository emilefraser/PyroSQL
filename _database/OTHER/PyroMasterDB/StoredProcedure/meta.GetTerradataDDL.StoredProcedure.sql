...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[meta].[GetTerradataDDL]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [meta].[GetTerradataDDL] AS' 
END
GO
...
{{META>}}
	...
	{Description}	Creates a generatemic TeraData DDL Statement

	{Usage>}		
					EXEC generate.GenerateTeraDataDdlStatement
	{<Usage}
{{<META}}
*/
ALTER     PROCEDURE [meta].[GetTerradataDDL]
AS
BEGIN
