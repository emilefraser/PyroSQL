...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[construct].[sp_GenMerge]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [construct].[sp_GenMerge] AS' 
END
GO
/***********************************************************************************************************************************************
Autor:		 Michal Golos http://toolien.pl/sp_genmerge/
...
EXEC dbo.sp_GenMerge
   ...
   , @debug                  = 0
***********************************************************************************************************************************************/
ALTER PROCEDURE [construct].[sp_GenMerge](
