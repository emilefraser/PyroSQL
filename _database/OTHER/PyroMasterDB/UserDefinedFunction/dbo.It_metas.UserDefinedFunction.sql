...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[It_metas]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
BEGIN
...
/*
  ...
  SELECT      [Result] =
    ...
    FROM      dbo.It_metas(N''''Number'''', 1) F

  ORDER BY    F.ThePath

  SELECT * FROM dbo.It_metas(''dbo.Number'', 1)

*/

CREATE FUNCTION [dbo].[It_metas] (@ObjectName VARCHAR(200), @ObjectsOnWhichItmetas BIT)
...
History: >
  ...
  https://www.red-gate.com/simple-talk/wp-content/uploads/imported/2284-Itmetas.html

compatibility: SQL Server 2005 - SQL Server 2017
Example usage: |

 /*
 * AdventureWorks.Employee: objects that meta on it
 ...
 */
  ...
  SELECT      [Result] =
    ...
    FROM      dbo.It_metas(N''''Employee'''', 0) F

  ORDER BY    F.ThePath

  /*
   * AdventureWorks.Employee: objects that it metas on
   * Note: function SPACE(...) indents rows to indicate hierarchical
   * relationships
   */
  ...
  SELECT      [Result] =
    ...
    FROM      dbo.It_metas(N''''Employee'''', 1) F

  ORDER BY    F.ThePath
Blog post overview and comments: https://www.red-gate.com/simple-talk/sql/t-sql-programming/metaencies-and-references-in-sql-server/
Revisions:
 - Author: Phil Factor
   Version: 1.1
   Modification: Allowed both types of metaencies, returned full detail table
   date: 2015-09-20
 ...
 - Author: Kevin Schultz 
   ...
   Date: 2016-03-28 
   Modification: >
     Add the schema name consistently to column "@Databasemetaencies.TheReferredEntity" by adding
 ...
 - Author: Kevin Schultz 
   ...
   Date: 2016-10-25 
   Modification: >   
     Insert the current metaency object type (Databasemetaencies.EntityType) instead of the
     parent metaency object type  (previousReferences.TheType), in the INSERT INTO SELECT clause

 - Author: Kevin Schultz 
   Version: 1.3
   Date: 2018-11-25 
   Modification: >
     2018-11-25 KLS Column sys.sql_expression_metaencies.referenced_id is NULLable, so substitute 
     with lookup OBJECT_ID(sys.sql_expression_metaencies.referenced_entity_name) when inserting 
     soft metaencies into table variable DatabaseDepencies. Sequences EntityType changed from SYN
 ...
 - Author: Kevin Schultz 
   ...
   Date:  2018-11-27 
   Modification: > 
    SQL inline table-valued function (''''IF'''') type added  as acceptable soft metaency
...
Returns: >
 ...
 of each metaent object
 In this example output, we have set @ObjectName=ObjectTableName and @ObjectsOnWhichItmetas=0,
 to retrieve objects metaent on ObjectTableName):
...
AS
  BEGIN
    ...
    /*
 Populate @Databasemetaencies via multiple UNION ALL statements, one for
 each hard and soft metaency type. Table of hard references:
 https://www.red-gate.com/simple-talk/wp-content/uploads/imported/2284-metaencies.png
 */
