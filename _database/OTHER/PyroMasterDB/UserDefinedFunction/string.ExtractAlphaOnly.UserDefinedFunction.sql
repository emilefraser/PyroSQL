...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[string].[ExtractAlphaOnly]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
BEGIN
execute dbo.sp_executesql @statement = N'CREATE FUNCTION [string].[ExtractAlphaOnly] (@pString varchar(8000))
...
--===== Autonomous
 ...
 FROM string.ExtractAlphaOnly(@pString) ca;
--===== CROSS APPLY example
 SELECT ca.udf_AlphaOnly
 FROM dbo.SomeTable st
 CROSS APPLY string.ExtractAlphaOnly(st.SomeVarcharCol) ca;
Programmer''s Notes:
...
--===== 1. Basic use against a literal
 ...
 FROM string.ExtractAlphaOnly(''xxx123abc999!!!'') ao;
--===== 2. Against a table 
 ...
 FROM @sampleTxt st
 CROSS APPLY string.ExtractAlphaOnly(st.txt);
---------------------------------------------------------------------------------------
Revision History:
