...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[tool].[remove_comments]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
BEGIN
...
*/    
CREATE   FUNCTION [tool].[remove_comments]  (@str VARCHAR(MAX))   RETURNS VARCHAR(MAX)    AS  
 BEGIN  
   declare     @i bigint=1   
    , @j bigint=null    
