...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[string].[GetStringContains]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
BEGIN
...
*/
CREATE   FUNCTION [string].[GetStringContains]
(
    @String VARCHAR(MAX),
    @substring XML,
    @start INT = NULL,
