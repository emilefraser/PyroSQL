...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[string].[CalcualteDamLev]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
BEGIN
...
{{##
	...
	(Usage)	
					SELECT * FROM  [string].[CalcualteDamLev](@s, @t, @max)
	(/Usage)
##}}
*/

CREATE   FUNCTION [string].[CalcualteDamLev](
