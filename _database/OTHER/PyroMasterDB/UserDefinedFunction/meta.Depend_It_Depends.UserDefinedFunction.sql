...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[meta].[Depend_It_Depends]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
BEGIN
...
/*
  ...
  SELECT      [Result] =
    ...
    FROM      dbo.It_Depends(N''''Number'''', 1) F

  ORDER BY    F.ThePath

  SELECT * FROM dbo.It_Depends(''dbo.Number'', 1)

*/

CREATE FUNCTION [meta].[Depend_It_Depends] (@ObjectName VARCHAR(200), @ObjectsOnWhichItDepends BIT)
