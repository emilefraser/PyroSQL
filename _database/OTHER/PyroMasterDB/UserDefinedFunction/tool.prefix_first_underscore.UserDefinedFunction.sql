...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[tool].[prefix_first_underscore]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
BEGIN
...
*/    
CREATE   FUNCTION [tool].[prefix_first_underscore]( @column_name VARCHAR(255) ) 
RETURNS VARCHAR(255) 
AS
BEGIN
