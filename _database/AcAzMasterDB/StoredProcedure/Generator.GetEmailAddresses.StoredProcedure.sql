...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[Generator].[GetEmailAddresses]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [Generator].[GetEmailAddresses] AS' 
END
GO
ALTER   PROCEDURE [Generator].[GetEmailAddresses]
	@Count AS BIGINT
AS
/******************************************************************************
