...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SplitSingleValueIntoRows]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[SplitSingleValueIntoRows] AS' 
END
GO
-- SPLITS a CLOB or single value into rows
-- EXEC [dbo].[SplitSingleValueIntoRows]
ALTER   PROCEDURE [dbo].[SplitSingleValueIntoRows]--(
