...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dim].[GenerateDateAndTimeFormat]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dim].[GenerateDateAndTimeFormat] AS' 
END
GO
...
{{META>}}
	...
	{Usage>}		
					...
					EXEC [dim].[GenerateDateAndTimeFormat] @DateTimeValue = @CurrentDT
	{<Usage}

	{Result}		SELECT * FROM [dim].[DateAndTimeFormat]
									
{{<META}}
--*/
ALTER   PROCEDURE [dim].[GenerateDateAndTimeFormat]
