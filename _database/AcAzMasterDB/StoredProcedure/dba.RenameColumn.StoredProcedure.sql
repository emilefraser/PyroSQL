...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dba].[RenameColumn]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dba].[RenameColumn] AS' 
END
GO

/*
EXEC [tool].[RenameColumn]
			@schemaname			=	'adf'
		,	@tablename			=	'LoadConfig'
		,	@columnname			=	'SourceIntegrationRuntime'
		,	@columnrename		=	'SourceGateway'
		,	@datatype			=	'SYSNAME'
		,	@executionmode		=	0

*/
ALTER   PROCEDURE [dba].[RenameColumn]
(
  @schemaname SYSNAME
  ,@tablename SYSNAME
  ,@columnname SYSNAME
...
-- Foreign key section
-- Drop foreign key
INSERT INTO #tmp_uRenameColumn (schemaname
, tablename
, objecttype
, operationtype
, sqltext)
	...
	JOIN sys.objects AS objp
		ON objp.object_id = fkc.parent_object_id
	JOIN sys.schemas AS schemap
		ON objp.schema_id = schemap.schema_id
	JOIN sys.objects AS objr
		ON objr.object_id = fkc.referenced_object_id
	JOIN sys.schemas AS schemar
		ON objr.schema_id = schemar.schema_id
	JOIN sys.columns AS colr
		ON colr.column_id = fkc.referenced_column_id
			AND colr.object_id = fkc.referenced_object_id
	JOIN sys.columns AS colp
		ON colp.column_id = fkc.parent_column_id
			AND colp.object_id = fkc.parent_object_id
	JOIN sys.objects AS constr
		ON constr.object_id = fkc.constraint_object_id
	WHERE
	-- ToDo
	...
	AND (objr.type = 'U'))
	OR ((schemap.name = @schemaname)
	AND (objp.name = @tablename)
	AND (colp.name = @columnname)
	AND (objr.type = 'U'));
...
(SELECT
	...
	JOIN INFORMATION_SCHEMA.KEY_COLUMN_USAGE AS KCU2
		ON (U.TABLE_CATALOG = KCU2.TABLE_CATALOG)
...
(SELECT
	...
	FROM UQC_PK
	WHERE (UQC_PK.rowtype = 'R')
	GROUP BY UQC_PK.objecttype
			,UQC_PK.TABLE_SCHEMA
			,UQC_PK.TABLE_NAME
			,UQC_PK.CONSTRAINT_NAME)
INSERT INTO #tmp_uRenameColumn (schemaname
...
, sqltext)
	...
	FROM UQC_PK2
	JOIN UQC_PK
		ON (UQC_PK.CONSTRAINT_NAME = UQC_PK2.CONSTRAINT_NAME)
...
, sqltext)
	...
	JOIN sys.check_constraints AS CHK
		ON CCU.CONSTRAINT_NAME = CHK.name
	...
	DISTINCT
	   ...
	   ,('ALTER TABLE [' + RTRIM(CCU.TABLE_SCHEMA) + '].[' + RTRIM(CCU.TABLE_NAME) + '] ' +
		CASE (CHK.is_not_trusted)
			...
			WHEN 1 THEN 'WITH NOCHECK ADD CONSTRAINT [' + RTRIM(CCU.CONSTRAINT_NAME) + '] CHECK ' + RTRIM(CHK.Definition) + ';'
		END) AS sqltext
	FROM INFORMATION_SCHEMA.CONSTRAINT_COLUMN_USAGE AS CCU
	JOIN sys.check_constraints AS CHK
		ON CCU.CONSTRAINT_NAME = CHK.name
...
, sqltext)
	...
	FROM sys.stats_columns AS statc
	JOIN sys.stats AS stat
		ON ((stat.stats_id = statc.stats_id)
				AND (stat.object_id = statc.object_id))
	JOIN sys.objects AS obj
		ON statc.object_id = obj.object_id
	JOIN sys.columns AS col
		ON ((col.column_id = statc.column_id)
...
(SELECT
	...
	SELECT
	   ...
	   ,CAST(S.SQLStr +
		...
		'] ' AS VARCHAR(MAX)) AS SQLStr
	FROM Stat AS S
	JOIN sys.stats_columns AS statcol
		ON ((statcol.object_id = S.object_id)
		AND (statcol.stats_id = S.stats_id))
	JOIN sys.columns AS col
		ON ((col.column_id = statcol.column_id)
		AND (col.object_id = statcol.object_id))
	WHERE (statcol.stats_column_id = (S.KeyOrdinal + 1))),
Stat2
AS
(SELECT
...
, sqltext)
	...
	FROM Stat2
	JOIN Stat
		ON ((Stat.object_id = Stat2.object_id)
...
, sqltext)
	...
	FROM sys.index_columns AS idxc
	JOIN sys.indexes AS idx
		ON ((idx.index_id = idxc.index_id)
				AND (idx.object_id = idxc.object_id))
	JOIN sys.objects AS obj
		ON idxc.object_id = obj.object_id
	JOIN sys.columns AS col
		ON ((col.column_id = idxc.column_id)
...
(SELECT
	...
	FROM (SELECT
		...
		AND NOT EXISTS (SELECT
			...
			AND (idxc.index_id = ic.index_id))) AS T
	...
	SELECT
	   ...
	   ,I.TableName
	   ,CAST(I.sqltext +
		CASE (idxcol.key_ordinal)
			WHEN 1 THEN ''
			ELSE ','
		END + ' [' + RTRIM(col.name) + '] ' AS VARCHAR(MAX)) AS sqltext
	FROM Create_Indexes AS I
	JOIN sys.index_columns AS idxcol
		ON ((idxcol.object_id = I.object_id)
		AND (idxcol.index_id = I.index_id))
	JOIN sys.columns AS col
		ON ((col.column_id = idxcol.column_id)
		AND (col.object_id = idxcol.object_id))
	WHERE (idxcol.key_ordinal = I.KeyOrdinal + 1)),
Create_Indexes2
AS
(SELECT
	...
	FROM Create_Indexes
	JOIN sys.objects AS O
		ON (O.object_id = Create_Indexes.object_id)
	WHERE (Create_Indexes.RowType = 'R')
	GROUP BY Create_Indexes.object_id
			,Create_Indexes.index_id)
INSERT INTO #tmp_uRenameColumn (schemaname
, tablename
, objecttype
...
, sqltext)
	...
	FROM Create_Indexes2
	JOIN Create_Indexes
		ON ((Create_Indexes.object_id = Create_Indexes2.object_id)
...
DECLARE C_SQL_DROP CURSOR LOCAL FORWARD_ONLY READ_ONLY FOR SELECT
	sqltext
FROM #tmp_uRenameColumn
WHERE (objecttype = 'FK')
AND (operationtype = 'D')
UNION ALL
SELECT
...
, operationtype
, sqltext)
	VALUES (@schemaname, @tablename, 'COL', 'A', @sqlaltertable);
...
IF (@columnname <> @columnrename)
	AND (LTRIM(RTRIM(@columnrename)) <> '')
BEGIN
...
, operationtype
, sqltext)
	VALUES (@schemaname, @tablename, 'COL', 'R', @sqlrenametable);
...
-- Statistics
--------------------------------------------------------
DECLARE C_SQL_CREATE CURSOR LOCAL FORWARD_ONLY READ_ONLY FOR SELECT
	sqltext
FROM #tmp_uRenameColumn
WHERE (objecttype = 'FK')
AND (operationtype = 'C')
