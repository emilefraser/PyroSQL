...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[meta].[GetAllPrimaryKeys]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [meta].[GetAllPrimaryKeys] AS' 
END
GO
/*
...
ALTER    PROCEDURE [meta].[GetAllPrimaryKeys]
...
BEGIN
	...
	BEGIN
		...
		SET @TSQLScripCreationIndex =
									  ...
									  + '.'
									  + QUOTENAME(REPLACE(@TableName, '_old',''))
									  + ' ADD CONSTRAINT '
									  + QUOTENAME(@IndexName)
									  + @is_unique_constraint
									  + @is_primary_key
