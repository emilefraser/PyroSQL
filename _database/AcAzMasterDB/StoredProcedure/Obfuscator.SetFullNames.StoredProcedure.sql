...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[Obfuscator].[SetFullNames]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [Obfuscator].[SetFullNames] AS' 
END
GO
ALTER   PROCEDURE [Obfuscator].[SetFullNames]
