...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[Generator].[GetPhoneNumbers]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [Generator].[GetPhoneNumbers] AS' 
END
GO
ALTER   PROCEDURE [Generator].[GetPhoneNumbers]
	@Count BIGINT
AS
/******************************************************************************
