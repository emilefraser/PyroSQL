...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[Generator].[GetAlphaNumericData]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [Generator].[GetAlphaNumericData] AS' 
END
GO
ALTER   PROCEDURE [Generator].[GetAlphaNumericData]
