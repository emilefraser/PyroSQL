...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[tool].[GetForeignKey]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
BEGIN
...
/*
	-- FK FROM MARC
	SELECT * FROM tool.GetForeignKey(''vs_lnd'', ''MARC_plant_data_for_material'' ,NULL, NULL)

	-- FK TO MARC
	SELECT * FROM tool.GetForeignKey(NULL, NULL, ''vs_lnd'', ''MARC_plant_data_for_material'')

*/

CREATE   FUNCTION [tool].[GetForeignKey](
