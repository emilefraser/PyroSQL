...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[tool].[sp_alter_column]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [tool].[sp_alter_column] AS' 
END
GO

ALTER PROCEDURE [tool].[sp_alter_column]
(
  @schemaname SYSNAME
  ,@tablename SYSNAME
  ,@columnname SYSNAME
