...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[inout].[ExtractLinkedServerData]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [inout].[ExtractLinkedServerData] AS' 
END
GO
...
*/
ALTER   PROCEDURE [inout].[ExtractLinkedServerData]
...
BEGIN
	...
	SET @sql_statement = 'SELECT * FROM OPENQUERY(' + @LinkedServerName + ', ''SELECT * FROM ' + @DatabaseName + '.' + @SchemaName + '.' + @ObjectName + ''')'
