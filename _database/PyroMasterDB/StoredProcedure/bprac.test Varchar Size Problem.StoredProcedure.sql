...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[bprac].[test Varchar Size Problem]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [bprac].[test Varchar Size Problem] AS' 
END
GO
ALTER   PROCEDURE [bprac].[test Varchar Size Problem]
AS
BEGIN
