...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[tool].[HelpExpandView]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [tool].[HelpExpandView] AS' 
END
GO
ALTER PROCEDURE [tool].[HelpExpandView] (
