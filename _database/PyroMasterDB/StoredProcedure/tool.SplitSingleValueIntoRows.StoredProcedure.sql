...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[tool].[SplitSingleValueIntoRows]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [tool].[SplitSingleValueIntoRows] AS' 
END
GO

-- SPLITS a CLOB or single value into rows
-- EXEC [dbo].[SplitSingleValueIntoRows]
ALTER     PROCEDURE [tool].[SplitSingleValueIntoRows]--(
