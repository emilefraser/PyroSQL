...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[tool].[CreateDatabaseIfNotExists]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [tool].[CreateDatabaseIfNotExists] AS' 
END
GO
...
/*
	EXEC [tool].[CreateDatabaseIfNotExists]
					@DatabaseName		= 'AdventureWorks_DV'
				,	@RootFolder			= 'D:\Database\localdb'
				,	@Category			= 'DataVault'
*/

ALTER        PROCEDURE [tool].[CreateDatabaseIfNotExists]
