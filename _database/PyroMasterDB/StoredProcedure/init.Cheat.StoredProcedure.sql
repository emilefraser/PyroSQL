...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[init].[Cheat]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [init].[Cheat] AS' 
END
GO

ALTER     PROCEDURE [init].[Cheat]
AS 
BEGIN
