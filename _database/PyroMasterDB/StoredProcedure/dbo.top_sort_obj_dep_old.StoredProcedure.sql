...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[top_sort_obj_dep_old]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[top_sort_obj_dep_old] AS' 
END
GO
...
ALTER   PROCEDURE [dbo].[top_sort_obj_dep_old]
