...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dataprofile].[ProfileData]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dataprofile].[ProfileData] AS' 
END
GO
/*
EXEC [dataprofile].[ProfileData] 
					   @Report = 1 --1 = 'ColumnDataProfiling'
...
ALTER   PROCEDURE [dataprofile].[ProfileData]
