...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[tool].[CreateSchemaIfNotExists]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [tool].[CreateSchemaIfNotExists] AS' 
END
GO
...
ALTER     PROCEDURE [tool].[CreateSchemaIfNotExists]
	@SchemaName SYSNAME
AS
BEGIN
