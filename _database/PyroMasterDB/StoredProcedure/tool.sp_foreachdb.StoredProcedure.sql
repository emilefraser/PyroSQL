...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[tool].[sp_foreachdb]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [tool].[sp_foreachdb] AS' 
END
GO

ALTER PROCEDURE [tool].[sp_foreachdb]
