...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dimension].[GenerateDateAndTimeFormat_Ext]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dimension].[GenerateDateAndTimeFormat_Ext] AS' 
END
GO
-- EXEC dim.GenerateDateAndTimeFormat_Ext
ALTER     PROCEDURE [dimension].[GenerateDateAndTimeFormat_Ext]
AS
BEGIN
