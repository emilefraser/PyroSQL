...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dimension].[TransferObjectToSchema]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dimension].[TransferObjectToSchema] AS' 
END
GO
...
/*
	...
	EXECUTION:
						EXEC [tool].[TransferObjectToSchema] 
							...
							,	@TargetSchemaName	= 'dimension'
							,	@ObjectTypeInclude	= NULL
							--,	@ObjectTypeExclude	= NULL
*/
ALTER   PROCEDURE [dimension].[TransferObjectToSchema]
...
BEGIN
	...
	--AND
	--	obj.name !=  IIF(@filter_exclude IS NULL, obj.name , @filter_exclude)
	AND
