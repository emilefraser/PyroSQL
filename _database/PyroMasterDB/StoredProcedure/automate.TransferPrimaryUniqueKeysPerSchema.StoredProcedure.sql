...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[automate].[TransferPrimaryUniqueKeysPerSchema]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [automate].[TransferPrimaryUniqueKeysPerSchema] AS' 
END
GO


/*
	EXEC [adf].[TransferPrimaryUniqueKeysPerSchema] @SchemaNameFilter = 'vs_lnd', @TableStringFilter = '_old'
*/

ALTER      PROCEDURE [automate].[TransferPrimaryUniqueKeysPerSchema]
	@SchemaNameFilter       SYSNAME
,	@TableStringFilter		NVARCHAR(100) = ''
AS
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			--SELECT @ColumnName, @IsDescendingKey,@IsIncludedColumn
		...
		SET @TSQLScripCreationIndex =
									  'ALTER TABLE ' +  QUOTENAME(@SchemaName) + '.' + QUOTENAME(REPLACE(@TableName, @TableStringFilter, ''))
									  + ' ADD CONSTRAINT '
									  + QUOTENAME(@IndexName + '_new')
									  + @is_unique_constraint
									  + @is_primary_key
									  + @IndexTypeDesc
									  + '('
		...
		IF NOT EXISTS  (SELECT * FROM sys.objects where name = @IndexName + '_new')
		BEGIN
