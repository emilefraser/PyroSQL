...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[construct].[CreateSchemaIfNotExists]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [construct].[CreateSchemaIfNotExists] AS' 
END
GO
...
ALTER     PROCEDURE [construct].[CreateSchemaIfNotExists]
	@SchemaName SYSNAME
AS
BEGIN
