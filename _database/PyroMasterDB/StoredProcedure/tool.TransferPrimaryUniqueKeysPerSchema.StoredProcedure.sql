...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[tool].[TransferPrimaryUniqueKeysPerSchema]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [tool].[TransferPrimaryUniqueKeysPerSchema] AS' 
END
GO
...
ALTER      PROCEDURE [tool].[TransferPrimaryUniqueKeysPerSchema]
