...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[init].[GenerateNumberTable]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [init].[GenerateNumberTable] AS' 
END
GO

/*
	EXEC [init].[GenerateNumberTable]
					@low	= 0
				,	@high	= 1000
*/
ALTER    PROCEDURE [init].[GenerateNumberTable]
	@low	INT		= 0
,	@high	INT		= 1000
AS
BEGIN
