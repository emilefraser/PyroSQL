...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[generate].[GetAlphaNumericData]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [generate].[GetAlphaNumericData] AS' 
END
GO
ALTER     PROCEDURE [generate].[GetAlphaNumericData]
	@AlphaNumericData AS dbo.DataEntity READONLY
AS
/******************************************************************************
* Name     : generate.GetAlphaNumericData
