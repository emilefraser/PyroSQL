...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[tool].[TruncateSchema]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [tool].[TruncateSchema] AS' 
END
GO

-- CREATES OR EXTENDS THE MODELLING SCHEMA DATABASE
-- EXEC [adf].[TruncateSchema] @Schema = 'ext'
ALTER       PROCEDURE [tool].[TruncateSchema]
	@Schema SYSNAME
AS
BEGIN
