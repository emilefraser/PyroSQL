...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[secure].[SetSSNs]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [secure].[SetSSNs] AS' 
END
GO
ALTER     PROCEDURE [secure].[SetSSNs]
...
INSERT INTO #FakeData EXEC generate.GetSSNs
	@Count = @ValueCount, 
