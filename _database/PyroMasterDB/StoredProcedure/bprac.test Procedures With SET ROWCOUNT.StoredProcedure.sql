...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[bprac].[test Procedures With SET ROWCOUNT]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [bprac].[test Procedures With SET ROWCOUNT] AS' 
END
GO
ALTER   PROCEDURE [bprac].[test Procedures With SET ROWCOUNT]
AS
BEGIN
