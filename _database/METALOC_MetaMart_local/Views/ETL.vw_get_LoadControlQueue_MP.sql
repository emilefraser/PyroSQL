...
CREATE VIEW [ETL].[vw_get_LoadControlQueue_MP] AS
SELECT
 ...
 WHERE 
	[control].QueuedForProcessingDT IS NOT NULL 
	AND [control].ProcessingState='Queued for load'
	AND [config].TargetDatabaseName IN ('ODS_X3P','ODS_Fleet_Data_Production','ODS_Tharisa_MiningData','ODS_X3V11','ODS_AMT','ODS_Integrove','ODS_LabWare','ODS_OPTIMIM','ODS_OptiMIMWeb'
