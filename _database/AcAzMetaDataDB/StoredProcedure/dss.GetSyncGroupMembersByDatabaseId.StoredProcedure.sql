...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dss].[GetSyncGroupMembersByDatabaseId]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dss].[GetSyncGroupMembersByDatabaseId] AS' 
END
GO
ALTER PROCEDURE [dss].[GetSyncGroupMembersByDatabaseId]
