...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[azure].[IngestAzureSchemaData]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [azure].[IngestAzureSchemaData] AS' 
END
GO
...
*/
ALTER   PROCEDURE [azure].[IngestAzureSchemaData]
AS
BEGIN
