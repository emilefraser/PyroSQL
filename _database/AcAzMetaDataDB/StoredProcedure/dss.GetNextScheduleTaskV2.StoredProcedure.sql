...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dss].[GetNextScheduleTaskV2]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dss].[GetNextScheduleTaskV2] AS' 
END
GO
ALTER PROCEDURE [dss].[GetNextScheduleTaskV2]
    @NoOfTasks int
AS
BEGIN
