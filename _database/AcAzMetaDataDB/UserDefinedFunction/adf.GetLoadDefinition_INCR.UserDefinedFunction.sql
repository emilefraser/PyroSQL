...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[adf].[GetLoadDefinition_INCR]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
BEGIN
execute dbo.sp_executesql @statement = N'
/*
	Created By: Emile Fraser
	Date: 2020-09-26
	Description: Gets the Load Definition for ADF Incremental load to extract data from source
	SELECT * FROM adf.LoadConfig WHERE SourceEntityName = ''EKKO''
	Test1: SELECT [adf].[GetLoadDefinition_INCR](180, ''SOURCE'')
	Test2: SELECT [adf].[GetLoadDefinition_INCR](5, ''TARGET'')

	TODO: Make the Waterfall field dynamic

	
*/
CREATE       FUNCTION [adf].[GetLoadDefinition_INCR] (
	@LoadConfigID			INT		
