...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[monitor].[MemoryUsageHistory]') AND type in (N'U'))
BEGIN
CREATE TABLE [monitor].[MemoryUsageHistory](
...
END
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[monitor].[DF_MemoryUsageHistory_DateStamp]') AND type = 'D')
BEGIN
ALTER TABLE [monitor].[MemoryUsageHistory] ADD  CONSTRAINT [DF_MemoryUsageHistory_DateStamp]  DEFAULT (getdate()) FOR [DateStamp]
END
GO
