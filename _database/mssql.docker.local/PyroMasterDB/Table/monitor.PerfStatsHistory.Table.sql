...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[monitor].[PerfStatsHistory]') AND type in (N'U'))
BEGIN
CREATE TABLE [monitor].[PerfStatsHistory](
...
END
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[monitor].[DF_PerfStatsHistory_StatDate]') AND type = 'D')
BEGIN
ALTER TABLE [monitor].[PerfStatsHistory] ADD  CONSTRAINT [DF_PerfStatsHistory_StatDate]  DEFAULT (getdate()) FOR [StatDate]
END
GO
