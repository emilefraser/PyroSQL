...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[tSQLt].[InstallExternalAccessKey]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [tSQLt].[InstallExternalAccessKey] AS' 
END
GO
ALTER PROCEDURE [tSQLt].[InstallExternalAccessKey]
AS
BEGIN
