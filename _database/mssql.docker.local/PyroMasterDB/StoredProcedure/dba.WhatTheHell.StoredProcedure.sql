...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dba].[WhatTheHell]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dba].[WhatTheHell] AS' 
END
GO
ALTER   PROCEDURE [dba].[WhatTheHell] 
...
BEGIN
    ...
    BEGIN
        /* Think of the StringToExecute as starting with this, but we will set this up later depending on whether we are doing an insert or a select:
