...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[test].[UnusedInputParameters]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [test].[UnusedInputParameters] AS' 
END
GO
...
ALTER PROCEDURE [test].[UnusedInputParameters]
