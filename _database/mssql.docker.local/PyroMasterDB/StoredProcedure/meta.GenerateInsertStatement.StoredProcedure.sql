...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[meta].[GenerateInsertStatement]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [meta].[GenerateInsertStatement] AS' 
END
GO
...
{{META>}}
	...
	{Description}	Creates a generatemic SQL Insert Statement

	{Usage>}		
						
					EXEC [meta].GenerateInsertStatement
									@tableName = 'dbo.Number'
								,	@GenerateSchema = 1

	{<Usage}
{{<META}}
*/
ALTER     PROCEDURE [meta].[GenerateInsertStatement]
AS
BEGIN
DECLARE 
      @tableName            NVARCHAR(128)   = 'dbo.Number'
