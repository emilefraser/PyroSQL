...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[meta].[CompleteDependencyMap]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [meta].[CompleteDependencyMap] AS' 
END
GO

/*
	EXEC dbo.metas_CompletemetaencyMap
*/
ALTER     PROCEDURE [meta].[CompleteDependencyMap]
AS
BEGIN
...
-- variables for referenced type obtained from sys.sql_expression_metaencies
declare @obj int
set @obj = 20
declare @type int
set @type = 21
...
IF OBJECT_ID('tempdb.dbo.#t2', 'U') IS NOT NULL
  DROP TABLE #t2;
-- we need to create another temporary table to store the metaencies from sys.sql_expression_metaencies till the updated values are inserted finally into #t1
create table #t2
(
	object_id int NULL,
	object_name sysname collate database_default NULL,
...
begin
	...
	begin
		-- HARD metaENCIES
		-- these metaencies have to be in the same database only
	...
	begin
		-- SOFT metaENCIES
		-- insert all values from sys.sql_expression_metaencies for the corresponding object
		...
		insert #t2 (object_type, object_name, object_schema, object_db, object_svr, relative_id, relative_name, relative_schema, relative_db, relative_type, schema_bound, rank)
			...
			from #t1 as t
			join sys.sql_expression_metaencies as dep on dep.referencing_id = t.object_id
			where @iter_no = t.rank and t.object_svr IS NULL and t.object_db = db_name()

		-- insert all the metaency values in case of a table that references a check
		insert #t2 (object_type, object_name, object_schema, object_db, object_svr, relative_id, relative_name, relative_schema, relative_db, relative_type, schema_bound, rank)
			select 
				case dep.referenced_class when 1 then @obj
				when 6 then @type
			...
			dep.is_schema_bound_reference, @iter_no + 1
			from #t1 as t
			join sys.sql_expression_metaencies as d on d.referenced_id = t.object_id
			join sys.objects as o on o.object_id = d.referencing_id and o.type = 'C'
			join sys.sql_expression_metaencies as dep on dep.referencing_id = d.referencing_id and dep.referenced_id != t.object_id
			where @iter_no = t.rank and t.object_svr IS NULL and t.object_db = db_name() and t.object_type = @u

		-- insert all the metaency values in case of an object that belongs to another object whose metaencies are being found
		insert #t2 (object_type, object_name, object_schema, object_db, object_svr, relative_id, relative_name, relative_schema, relative_db, relative_type, schema_bound, rank)
			select
				case dep.referenced_class when 1 then @obj
				when 6 then @type
			...
			join sys.sql_expression_metaencies as dep on dep.referencing_id = o.object_id
			where @iter_no = t.rank and t.object_svr IS NULL and t.object_db = db_name()
		...
		BEGIN
			...
			-- the parent object will have the default schema of user in case of generatemic schema binding
			-- @obj
			SET @query = 'update #t2 set object_db = N' + quotename(@dbname, '''') + ', object_id = obj.object_id, object_schema = SCHEMA_NAME(obj.schema_id), object_type = 
							case when obj.type = ''U'' then ' + CAST(@u AS nvarchar(8)) +
							' when obj.type = ''V'' then ' + CAST(@v AS nvarchar(8)) +
	...
	update #t2 set object_type = @unknown where object_id IS NULL
		...
		-- uddt referenced by sequence. Used to find UDDT that is in sequence metaencies.
		insert #t1 (object_id, object_name, object_schema, object_db, object_type, relative_id, relative_name, relative_schema, relative_db, relative_type, schema_bound, rank)
			select tp.user_type_id, tp.name, SCHEMA_NAME(tp.schema_id), t.object_db, case tp.is_assembly_type when 1 then @udt else @uddt end, t.object_id, t.object_name, t.object_schema, t.object_db, t.object_type, 1, @iter_no + 1
			from #t1 as t
