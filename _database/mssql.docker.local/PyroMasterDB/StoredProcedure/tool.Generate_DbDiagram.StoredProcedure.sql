...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[tool].[Generate_DbDiagram]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [tool].[Generate_DbDiagram] AS' 
END
GO
...
ALTER     PROCEDURE [tool].[Generate_DbDiagram]
