...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dba].[WTF]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dba].[WTF] AS' 
END
GO

ALTER PROCEDURE [dba].[WTF] 
...
,		@StringToExecute        VARCHAR(MAX)
,		@OutputTableCleanupDate DATE
,		@SessionWaits           BIT            = 0
,		@SessionWaitsSQL        NVARCHAR(MAX)  = N'LEFT JOIN ( SELECT DISTINCT
												    wait.session_id ,
...
,		@QueryStatsXMLSQL       NVARCHAR(MAX)  = N'OUTER APPLY sys.dm_exec_query_statistics_xml(s.session_id) qs_live'
    ...
    IF EXISTS(SELECT         
			  ...
			  FROM         
				  sys.all_columns
			  WHERE
						  object_id = OBJECT_ID('sys.dm_exec_query_statistics_xml')
						  AND name = 'query_plan')
	BEGIN
		SET @QueryStatsXMLselect = N' CAST(COALESCE(qs_live.query_plan, ''<?No live query plan available. To turn on live plans, see https://www.BrentOzar.com/go/liveplans ?>'') AS XML) '
		SET @QueryStatsXMLSQL = N'OUTER APPLY sys.dm_exec_query_statistics_xml(s.session_id) qs_live'
	END
		ELSE
	BEGIN
		SET @QueryStatsXMLselect = N' NULL '
		SET @QueryStatsXMLSQL = N' '
	END
	SELECT 
		@BlockingCheck = N'SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED
	...
	BEGIN
		...
		SET @StringToExecute = N'
				...
				,	[database_name]			= COALESCE(DB_NAME(r.database_id), DB_NAME(blocked.dbid), ''N/A'')
				,	[query_text]			= ISNULL(SUBSTRING(dest.text,
													...
													( ( CASE query_stats.statement_end_offset
													 ...
													 END - query_stats.statement_start_offset )
													  / 2 ) + 1), dest.text)
				,	[query_plan]			= derp.query_plan
				,	[query_cost]			= qmg.query_cost									   		   
						       
			    ,	[blocking_session_id]	=	COALESCE(wt.wait_info, RTRIM(blocked.lastwaittype) + '' ('' + CONVERT(VARCHAR(10), 
		...
		SET @StringToExecute+='FROM sys.dm_exec_sessions AS s
			     LEFT JOIN sys.dm_exec_requests AS r
			     ON   r.session_id = s.session_id
			     LEFT JOIN ( SELECT DISTINCT
			          wait.session_id ,
			          ( SELECT waitwait.wait_type + '' (''
			     ...
			     WHERE s.session_id <> @@SPID 
				    ...
				    ' + CASE
							WHEN @ShowSleepingSPIDs = 0 THEN N' AND COALESCE(DB_NAME(r.database_id), DB_NAME(blocked.dbid)) IS NOT NULL'
							WHEN @ShowSleepingSPIDs = 1 THEN N' OR COALESCE(r.open_transaction_count, blocked.open_tran) >= 1'
							ELSE N''
						END
	END
	...
	BEGIN
		SELECT 
			@EnhanceFlag = CASE
					...
					AND @ProductVersionMinor >= 6020 THEN 1
							   WHEN
					@ProductVersionMajor = 12
					AND @ProductVersionMinor >= 5000 THEN 1
							   WHEN
					@ProductVersionMajor = 13
					AND @ProductVersionMinor >= 1601 THEN 1
		...
		SELECT 
			@StringToExecute = N' 
			          ...
			          , qmg.query_cost
			           ,s.status ,
			           COALESCE(wt.wait_info, RTRIM(blocked.lastwaittype) + '' ('' + CONVERT(VARCHAR(10), blocked.waittime) + '')'' ) AS wait_info ,' + CASE @SessionWaits
		...
		BEGIN
            ...
            END AS transaction_isolation_level ,
		            qmg.dop AS degree_of_parallelism ,	' 
            + CASE @EnhanceFlag WHEN 1 THEN N'query_stats.last_dop,
            query_stats.min_dop,
            query_stats.max_dop,
	...
	BEGIN
		...
		SET @StringToExecute+=N' ) '
	END
	SET @StringToExecute+=N' ORDER BY 2 DESC
	    '
