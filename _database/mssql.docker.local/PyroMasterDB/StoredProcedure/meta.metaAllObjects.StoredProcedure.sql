...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[meta].[metaAllObjects]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [meta].[metaAllObjects] AS' 
END
GO
/*
	EXEC meta.metaAllObjects 
		@objectName		= 'azure.ArmSchema'
	,	@objectClass	= N'OBJECT'
	,	@debug			= 1
*/
ALTER     PROCEDURE [meta].[metaAllObjects] (
