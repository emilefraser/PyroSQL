...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[internal].[ValidateTempTablesSchema]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [internal].[ValidateTempTablesSchema] AS' 
END
GO
...
--      error message.
-- =======================================================================
ALTER   PROCEDURE [internal].[ValidateTempTablesSchema]
