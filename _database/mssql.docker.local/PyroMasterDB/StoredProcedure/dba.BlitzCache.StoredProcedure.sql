...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dba].[BlitzCache]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dba].[BlitzCache] AS' 
END
GO

ALTER PROCEDURE [dba].[BlitzCache]
    @get_help BIT = 0,
    @top INT = 50,
