...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[construct].[GenerateCreateTableDDL_Foreign2]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [construct].[GenerateCreateTableDDL_Foreign2] AS' 
END
GO
ALTER   PROCEDURE [construct].[GenerateCreateTableDDL_Foreign2] 
...
IF CHARINDEX ('.',@TableName)=0 
	SELECT @TableName=QUOTENAME(Object_Schema_name(s.object_id))
	          +'.'+QUOTENAME(s.name)
	FROM sys.objects s WHERE s.name LIKE @TableName
...
SELECT @sql_statement=object_definition(OBJECT_ID(@TableName))
IF @sql_statement IS NULL
	IF (SELECT TYPE FROM sys.objects 
	    WHERE object_id=OBJECT_ID(@TableName))
      IN ('U','S')--if it is a table
		BEGIN
		SELECT @sql_statement='/*'+CONVERT(VARCHAR(2000),value)+'*/
'       FROM  sys.extended_properties ep
			WHERE ep.major_ID = OBJECT_ID(@TableName) 
			AND  minor_ID=0 AND class=1

SELECT @sql_statement=COALESCE(@sql_statement,'')+'CREATE TABLE '+@TableName+'(
   ' +
