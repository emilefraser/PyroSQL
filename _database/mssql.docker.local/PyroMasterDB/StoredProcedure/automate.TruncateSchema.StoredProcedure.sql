...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[automate].[TruncateSchema]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [automate].[TruncateSchema] AS' 
END
GO

-- CREATES OR EXTENDS THE MODELLING SCHEMA DATABASE
-- EXEC [adf].[TruncateSchema] @Schema = 'ext'
ALTER       PROCEDURE [automate].[TruncateSchema]
	@Schema SYSNAME
AS
BEGIN
	...
	SELECT 
		TableName =  rc.ObjectName
	,	SchemaName = rc.SchemaName
	FROM 
		measure.ObjectRowCountFromPartition AS rc
	WHERE
