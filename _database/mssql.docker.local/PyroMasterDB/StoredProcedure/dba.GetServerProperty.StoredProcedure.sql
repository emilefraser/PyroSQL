...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dba].[GetServerProperty]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dba].[GetServerProperty] AS' 
END
GO


ALTER   PROCEDURE [dba].[GetServerProperty]
AS 
