...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[String_or_binary_data_truncated]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[String_or_binary_data_truncated] AS' 
END
GO

ALTER   PROCEDURE [dbo].[String_or_binary_data_truncated]
@String VARCHAR(MAX)
AS
