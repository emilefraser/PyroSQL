...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[string].[GetString_or_binary_data_truncated2]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [string].[GetString_or_binary_data_truncated2] AS' 
END
GO

--–DROP PROCEDURE usp_String_or_binary_data_truncated
--–GO
ALTER     PROCEDURE [string].[GetString_or_binary_data_truncated2]
@String VARCHAR(MAX)
AS
