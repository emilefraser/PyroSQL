...
begin 
   ...
   if @is_running_in_batch=0 -- this push is running in a batch 
	 update dbo.Batch set batch_name = @transfer_name where batch_id = @batch_id 
	exec dbo.log @transfer_id , 'header', 'start ?(?) target table ?', @proc_name , @transfer_id, @target_table 
...
'
	...
	print @sql 
	exec dbo.log @transfer_id , 'footer', 'start ?(?) target table ?', @proc_name , @transfer_id, @target_table 
end
