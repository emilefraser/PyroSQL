...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[printer].[PrintDebug]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [printer].[PrintDebug] AS' 
END
GO

ALTER PROCEDURE [printer].[PrintDebug]
(
