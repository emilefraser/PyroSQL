...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[benchmark].[UpdateMetricsLog]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [benchmark].[UpdateMetricsLog] AS' 
END
GO

ALTER   PROCEDURE [benchmark].[UpdateMetricsLog]
	@LogID INT
AS
