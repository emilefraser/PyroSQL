...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[string].[GetStringOrBinaryTruncated2]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [string].[GetStringOrBinaryTruncated2] AS' 
END
GO
...
--GO
ALTER     PROCEDURE [string].[GetStringOrBinaryTruncated2]
	@String VARCHAR(MAX)
AS
