...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[secure].[SetFullNames]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [secure].[SetFullNames] AS' 
END
GO
ALTER     PROCEDURE [secure].[SetFullNames]
...
INSERT INTO #FakeData EXEC generate.GetFullNames
	@Count = @ValueCount;
