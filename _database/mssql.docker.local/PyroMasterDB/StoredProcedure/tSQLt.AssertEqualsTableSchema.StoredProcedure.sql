...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[tSQLt].[AssertEqualsTableSchema]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [tSQLt].[AssertEqualsTableSchema] AS' 
END
GO
ALTER PROCEDURE [tSQLt].[AssertEqualsTableSchema]
