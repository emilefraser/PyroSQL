...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[array].[ReplaceXmlArrayValueWithString]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
BEGIN
...
Phil'')
*/
CREATE   FUNCTION [array].[ReplaceXmlArrayValueWithString]
   (
