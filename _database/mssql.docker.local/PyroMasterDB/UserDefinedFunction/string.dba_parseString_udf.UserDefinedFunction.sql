...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[string].[dba_parseString_udf]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
BEGIN
execute dbo.sp_executesql @statement = N'
CREATE FUNCTION [string].[dba_parseString_udf]
(
...
Test Cases:
    ...
    1.  multiple strings separated by space
        SELECT * FROM string.dba_parseString_udf(''  aaa  bbb  ccc '', '' '');
