...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[string].[GetChunkFromSplitStringToRowsUsingDelimiter]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
BEGIN
...
/*
	...
	SELECT [string].[SplitStringToRowsUsingDelimiter] 
*/
CREATE   FUNCTION [string].[GetChunkFromSplitStringToRowsUsingDelimiter] (
...
RETURN
	...
	SELECT 
		RowValue = SUBSTRING(@StringValue, s.ChunkStart, ISNULL(NULLIF(CHARINDEX(@Delimiter, @StringValue, s.ChunkStart), 0) - s.ChunkStart, 8000))
--	,	ChunkStart
--	,	ChunkNumber
	FROM 
		cteStart s
