...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[string].[ExtractAlphaNumericOnly]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
BEGIN
execute dbo.sp_executesql @statement = N'CREATE FUNCTION [string].[ExtractAlphaNumericOnly] (@pString varchar(8000)) 
...
--===== Autonomous
 ...
 FROM string.ExtractAlphaNumericOnly(@pString) ca;

--===== CROSS APPLY example
 SELECT ca.udf_AlphaNumericOnly
 FROM dbo.SomeTable st
 CROSS APPLY string.ExtractAlphaNumericOnly(st.SomeVarcharCol) ca;
...
--===== 1. Basic use against a literal
 ...
 FROM string.ExtractAlphaNumericOnly(''xxx123abc999!!!'') ao;
...
--===== 2. Against a table 
 ...
 FROM @sampleTxt st
 CROSS APPLY string.ExtractAlphaNumericOnly(st.txt);
