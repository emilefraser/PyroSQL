...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[meta].[Depend_Depends]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
BEGIN
...
CREATE   FUNCTION [meta].[Depend_Depends] (
...
/*
	...
	FROM    dbo.Depends(''ArmTemplate'',0)
	ORDER BY ThePath
