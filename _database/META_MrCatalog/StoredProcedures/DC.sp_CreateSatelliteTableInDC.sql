...
CREATE PROCEDURE [DC].[sp_CreateSatelliteTableInDC] 
@HubID int
...
SELECT	DISTINCT  SatelliteName
				...
				, GETDATE()
				, [DataEntityTypeID] = 
					CASE WHEN SatelliteName LIKE 'REFSAT%'
						THEN [DC].[udf_get_DataEntityTypeID]('REFSAT')
						ELSE [DC].[udf_get_DataEntityTypeID]('SAT' + sat.SatelliteDataVelocityTypeCode)
					END 
...
SELECT DISTINCT NULL
	  ...
	  ,REPLACE(HubBKDataEntityHKName, 'REF_', '') AS HubBKDataEntityHKName
	  ,'varchar'
	  ,40
	  ,0
	  ,0
