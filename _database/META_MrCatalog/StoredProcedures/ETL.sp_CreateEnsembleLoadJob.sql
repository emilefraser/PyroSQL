...
DECLARE ensemble_cursor CURSOR FOR   
	...
	--EXEC [ETL].[sp_CreateEnsembleLoadJob] 12, 2
*/
...
/*
	...
	SELECT * FROM DMOD.Hub where IsActive = 1
	SELECT * FROM DC.[Database]
*/
...
FROM
	...
	-- NOTE: Frans commented this out because EMS is now being loaded incrementally
	--WHERE 
	--	StageSchemaName <> 'EMS'
...
IF EXISTS (SELECT job_id, [name] FROM msdb.dbo.sysjobs WHERE [name] = @JobName)
BEGIN
