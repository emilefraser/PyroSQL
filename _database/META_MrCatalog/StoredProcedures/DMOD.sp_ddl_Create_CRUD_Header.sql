...
-- =============================================
-- Sample Execution: DMOD.sp_ddl_Create_CRUD_Header @DataEntityID = 9906, @UniqueBusinessKeyColumn = 'SortOrderGroupCode', @PrimaryKeyColumn = 'SortOrderGroupingID'
CREATE PROCEDURE [DMOD].[sp_ddl_Create_CRUD_Header](
    @DataEntityID INT,
    @UniqueBusinessKeyColumn VARCHAR(100), -- the business key that is unique
    @PrimaryKeyColumn VARCHAR(100) -- primary key column of the table
    --if the UniqueBusinesskeyColumn and PrimaryKeyColumn are both used for the same purpose, enter the same column name in both parameters
	)
AS
