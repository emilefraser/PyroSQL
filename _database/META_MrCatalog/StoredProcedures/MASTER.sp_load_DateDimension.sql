...
SET @BeginDate = '2000/01/01' --DATEADD(YEAR,+2,CAST(GETDATE() AS DATE))
...
-- TRUNCATE table [MASTER].DateDimension
	...
	update [MASTER].DateDimension
	set [IsCurrentMonth] = CASE WHEN [MonthNumber] = DATEPART(MONTH,GETDATE())  AND [Year] = DATEPART(YEAR,GETDATE())  
					THEN 1 
					ELSE 0 
					END
