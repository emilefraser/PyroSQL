...
-- ====================================================================================================
CREATE PROCEDURE [DMOD].[sp_CreatePKFKLinkTableInDC_VirtualDC]
		@ChildHubID INT,
...
DECLARE @TargetSchemaID INT =
						(
							SELECT TOP(1) s.SchemaID 
										FROM 
										[DMOD].[Schema] s   --WSCHANGES
...
FROM	#PKFKTemp pkfk
INNER JOIN DMOD.DataEntity de ON pkfk.LinkName = de.DataEntityName --WS DC TO DMOD
	AND de.SchemaID = @TargetSchemaID
...
INSERT INTO DMOD.[Field] ([FieldName],[DataType],[MAXLENGTH],[Precision],[Scale],[DataEntityID],[CreatedDT],[IsActive],[FieldSortOrder] )
SELECT	   f.FieldName
		  ,f.DataType
		  ,f.[MAXLENGTH]
		  ,f.[Precision]
