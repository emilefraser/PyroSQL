...
CREATE PROCEDURE [DMOD].[PROD_GhostRecords_HubAndSatTables_Validation] 
...
-- use [DATAVAULT].
   ...
   set @sqlCommand =  'select '''+@Dataentityname+''','''+@fieldname+''' ,(select count(1) from [DATAVAULT].'+@Dataentityname+' ),(select count(1) from [DATAVAULT].'+@Dataentityname+' Where '+@fieldname+' = '+'''3FEDA0153EEE1380B496298450DC5A74324EB8C1'''+'),
   CASE
   WHEN (select count(1) from [DATAVAULT].'+@Dataentityname+' ) = 0 THEN 0
   ELSE
   (Cast(((select count(1) from [DATAVAULT].'+@Dataentityname+' Where '+@fieldname+' = '+'''3FEDA0153EEE1380B496298450DC5A74324EB8C1'''+'))as FLOAT)/cast(((select count(1) from [DATAVAULT].'+@Dataentityname+' )) as FLOAT))*100
   END AS Ghostpercent'
