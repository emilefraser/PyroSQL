...
CREATE PROCEDURE [DC].[sp_CreateSameAsLinkStageTableInDC] 
@HubID int,
...
--DECLARE @HubID INT
--SET @HubID = 1035
--DECLARE @TargetDatabaseID int = 3
--DECLARE @TargetSchemaName varchar (100) = 'EMS'
...
SELECT DISTINCT  SourceName + '_SAL'
				,@TargetSchemaID
				,GETDATE()
FROM	#SalDataEntity
WHERE NOT EXISTS
	 (SELECT 1
	  ...
	  INNER JOIN DC.DataEntity de ON
	  salt.SourceName + '_SAL' = de.DataEntityName
...
FROM	#SalDataEntity salt
INNER JOIN DC.DataEntity de ON salt.SourceName + '_SAL' = de.DataEntityName
	AND de.SchemaID = @TargetSchemaID
...
DECLARE @InitialSourceDataEntityName VARCHAR(100) = (SELECT DISTINCT SourceName FROM #SalDataEntity)
	INSERT #Fields VALUES
		  (NULL,'BKHash','varchar',40,0,0,@TargetDataEntityID, GETDATE(), 1, 1),
