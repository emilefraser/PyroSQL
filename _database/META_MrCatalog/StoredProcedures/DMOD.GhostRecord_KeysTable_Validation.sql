...
and de.DataEntityName NOT LIKE '%dbo_SalesInvoiceLine_EMS_KEYS%'
 ...
 --use [DEV_StageArea].
   ...
   set @sqlCommand =  'select '''+@Dataentityname+''','''+@fieldname+''' ,(select count(1) from [DEV_StageArea].'+@Dataentityname+' ),(select count(1) from [DEV_StageArea].'+@Dataentityname+' Where '+@fieldname+' = '+'''3FEDA0153EEE1380B496298450DC5A74324EB8C1'''+'),
   CASE
   WHEN (select count(1) from [DEV_StageArea].'+@Dataentityname+' ) = 0 THEN 0
   ELSE
   (Cast(((select count(1) from [DEV_StageArea].'+@Dataentityname+' Where '+@fieldname+' = '+'''3FEDA0153EEE1380B496298450DC5A74324EB8C1'''+'))as FLOAT)/cast(((select count(1) from [DEV_StageArea].'+@Dataentityname+' )) as FLOAT))*100
   END AS Ghostpercent'
