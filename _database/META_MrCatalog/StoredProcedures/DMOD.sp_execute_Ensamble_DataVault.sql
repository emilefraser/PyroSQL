...
/*
	...
	How? 
		...
		DECLARE @DatabaseName VARCHAR(150) = 'DataVault'
		
		EXEC DMOD.sp_execute_AllLoads_DataVault @Today, @IsTestLoad, @DatabaseName

*/

CREATE        PROCEDURE  [DMOD].[sp_execute_Ensamble_DataVault]
	@Today VARCHAR(100), @IsTestLoad BIT, @DatabaseName VARCHAR(150),
	@EntityWildcard VARCHAR(150) 
AS
...
AND 
(	
	Target_DEName LIKE '%SAT_' + @EntityWildcard + '_%'
OR 
	Target_DEName LIKE '%HUB_' + @EntityWildcard + '%'
OR
	Target_DEName LIKE 'LINK_%_' +  @EntityWildcard
)
...
BEGIN  
			...
			--	IF (@LoadClassification = 'FullLoad')
			--	BEGIN
			...
			--			INSERT INTO #LoadErrorLog_DV
			--			VALUES
			--			(@LoadConfigID,@Target_DatabaseName,@Target_SchemaName , @Target_TableName,'Truncate SUCCESS')
			...
			--	END
				...
				BEGIN
						INSERT INTO #LoadErrorLog_DV
						VALUES
						(@LoadConfigID,@Target_DatabaseName,@Target_SchemaName , @Target_TableName,'ERROR! There is not a Load Procedure in DataVault for LoadConfigID: '+ CONVERT(VARCHAR(MAX), @LoadConfigID))
