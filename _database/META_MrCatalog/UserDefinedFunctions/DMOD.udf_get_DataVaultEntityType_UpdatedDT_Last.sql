...
--!~ Last UpdatedT replacement for Incremental Loads
			SELECT	@LastUpdatedT = ISNULL(MAX([MODIFIEDDATETIME1]),'1900-01-01')
			FROM	[DEV_DataVault].[raw].[SAT_Customer_EMS_LVD]
-- Last UpdatedT replacement for Incremental Loads ~!

-- Sample Execution Statement
	--Select [DMOD].[udf_get_DataVaultEntityType_UpdatedDT_Last](25) 
*/
CREATE     FUNCTION [DMOD].[udf_get_DataVaultEntityType_UpdatedDT_Last](
	@LoadConfigID INT
)
RETURNS VARCHAR(MAX)
AS
BEGIN
	...
	DECLARE @UpdatedT_Last_FieldName VARCHAR(MAX)
	...
	-- Get UpdatedT Field 
	SET @UpdatedT_Last_FieldName = (SELECT PARSENAME(DMOD.udf_get_SatelliteUpdatedDT_Last_Field(@LoadConfigID), 1))

	-- For the TargetDataEntity Ensamble what is the that is being loaded?
	-- :DEBUG:
	--SELECT @UpdatedT_Last_FieldName, @TargetDataEntityName, @TargetSchemaName, @DataEntityType, @SourceSchemaName
	-- :DEBUG:
	...
	BEGIN
		...
		SET @Entity_Return_Name = (
										...
										WHERE
											f.FieldName = @UpdatedT_Last_FieldName
										AND
	...
	SET @ReturnName = REPLICATE(CHAR(9), 3) + '--!~ Last UpdatedT replacement for Incremental Loads' + CHAR(13)
	SELECT @ReturnName = @ReturnName + REPLICATE(CHAR(9), 4) + 'SELECT	@LastUpdatedT = ISNULL(MAX(' + QUOTENAME(@UpdatedT_Last_FieldName) + '),''1900-01-01'')' + CHAR(13)
	...
	SELECT @ReturnName = @ReturnName + REPLICATE(CHAR(9), 3) + '-- Last UpdatedT replacement for Incremental Loads ~!' + CHAR(13)
