...
--Select [DMOD].[udf_get_DataVault_LinkHKName](651) 
*/
CREATE   FUNCTION [DMOD].[udf_get_DataVault_ChildHubHKName](
	@LoadConfigID INT
)
RETURNS VARCHAR(MAX)
AS
BEGIN
	...
	BEGIN
		...
		BEGIN
				SET @ReturnName = (SELECT REPLACE(PARSENAME(@TargetDataEntityName, 1), 'LINK_', ''))
				SET @ReturnName = (SELECT 'HK_' + SUBSTRING(@ReturnName, CHARINDEX('_', @ReturnName) + 1, 150))
		END
	END
