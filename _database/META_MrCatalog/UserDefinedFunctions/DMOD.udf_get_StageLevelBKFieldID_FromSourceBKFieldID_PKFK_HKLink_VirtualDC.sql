...
CREATE FUNCTION [DMOD].[udf_get_StageLevelBKFieldID_FromSourceBKFieldID_PKFK_HKLink_VirtualDC]
(
...
BEGIN
    ...
    DECLARE @Result varchar(50),
		...
		--DECLARE @SourceFieldID int = 163307
				--, @VelocityType int = 2
				--, @ParentFieldName varchar(100) = '%SalesInvoiceHeader_StockTrail%'
	...
	from	
			(
				...
				from	DMOD.FieldRelation_VirtualDC frsource
		...
		inner join 
			(
				...
				from	DMOD.FieldRelation_VirtualDC frtarget
					inner join DMOD.Field_VirtualDC ftarget on frtarget.TargetFieldID = ftarget.FieldID
					inner join DMOD.DataEntity_VirtualDC detarget on detarget.DataEntityID = ftarget.DataEntityID
					inner join DMOD.[Schema_VirtualDC] schematarget on detarget.SchemaID = schematarget.SchemaID
					inner join DMOD.[Database_VirtualDC] dbtarget on dbtarget.DatabaseID = schematarget.DatabaseID
	...
	AS
		...
		where SourceFieldID = @SourceFieldID
			--AND cte.SourceFieldName = cte.ParentFieldName
			...
			AND cte.TargetDataEntityName like '%_KEYS' 
			AND cte.ParentFieldName like 'LINKHK%' 
			--AND cte.ParentFieldName like 
