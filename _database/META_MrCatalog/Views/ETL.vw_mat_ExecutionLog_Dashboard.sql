...
SELECT [ExecutionLogID] AS [Execution Log ID]
	  ...
	  ,CASE
		...
		*/
	   ,ISNULL(DatabasePurposeCode, 'Unknown') AS [Database Purpose] 
	   ,CASE
	  ...
	  ,CASE 
			WHEN [IsError] IS NULL THEN 'No'
			WHEN [IsError] = 0 THEN 'No'
			WHEN [IsError] = 1 THEN 'Yes'
			ELSE 'Unk' END AS [Is Error]
	  ,CASE 
			WHEN [IsDataIntegrityError] IS NULL THEN 'No'
			WHEN [IsDataIntegrityError] = 0 THEN 'No'
			WHEN [IsDataIntegrityError] = 1 THEN 'Yes'
			ELSE 'Unk' END AS [Is Data Integrity Error]
      ...
      ,[RowsTransferred] as [Rows Transfered]
	  ...
	  ,ISNULL([DetailTypeDescription], 'Unknown') as [Load Type Name]
	  ,ISNULL([DataEntityTypeCode], 'Unknown') AS [Data Entity Type Code]
	  ,ISNULL([DataEntityTypeName], 'Unknown') AS [Data Entity Type Name]
	  ,CASE 
			WHEN [IsLastRunOfConfigID] IS NULL THEN 'No' 
			WHEN [IsLastRunOfConfigID] = 0 THEN 'No' 
			WHEN [IsLastRunOfConfigID] = 1 THEN 'Yes' 
			ELSE 'Unk' END AS [Is Last Status]
	  	  , CASE 
