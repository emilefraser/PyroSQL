...
CREATE view [DC].[vw_DCDataLineage_DataEntity] as 
...
SELECT  distinct
  ...
  FROM [DC].[FieldRelation] fr
	  join dc.Field f 
		on f.FieldID = fr.SourceFieldID
	  join dc.Field f1 
		on f1.FieldID = fr.TargetFieldID 
	  join dc.DataEntity de 
		on de.DataEntityID = f.DataEntityID
	  join dc.DataEntity de1 
		on de1.DataEntityID = f1.DataEntityID
	  join dc.[schema] s 
