...
CREATE PROCEDURE [DC].[sp_CreateStageTableInDC_Backup20190917] 
	@HubID INT
...
INSERT INTO DC.FieldRelation
	...
	FROM	
		(
			...
			SELECT	 
					DISTINCT  CASE WHEN     h.HubName LIKE 'REF_%' THEN  DC.udf_get_ODSFieldID_From_SourceFieldID_ForRefTables(h.BKFieldID) -- FS - Added Field Relations for REF tables that are modelled as HUBS
					          ELSE DC.udf_get_ODSFieldID_From_SourceFieldID(h.BKFieldID)
							  END AS ODSFieldID
					, f.FieldID AS StageFieldID
