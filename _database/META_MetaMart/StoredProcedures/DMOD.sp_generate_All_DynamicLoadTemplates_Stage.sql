...
/*
	...
	How? 
		...
		EXEC DataManager.DMOD.[sp_generate_All_DynamicLoadTemplates_Stage] @Author, 'DEV', 1




		DECLARE @Author VARCHAR(100) = 'Frans Germishuizen'
		EXEC DataManager.DMOD.[sp_generate_All_DynamicLoadTemplates_Stage] @Author, 'PROD',0

*/

CREATE PROC  [DMOD].[sp_generate_All_DynamicLoadTemplates_Stage] 
				@Author VARCHAR(100) , @Environment varchar(10), @IsDebug BIT
AS
...
-- KEYS
DECLARE load_cursor_keys CURSOR FOR 
...
WHERE 1=1
	AND de.IsAllowedInStageArea = 1
	AND lc.config_IsActive = 1
	AND lc.loadtype_IsActive = 1 
	AND lc.LoadConfigID <> 261

	--AND lc.Source_DB = 'ODS_D365'
	--AND lc.Target_DEName LIKE '%SalesReturnLine%'
	--AND (LoadTypeCode = 'StageIncrementalWithHistoryUpdateLoad_KEYS'
	--OR LoadTypeCode = 'StageIncrementalWithHistoryUpdateLoad_MVD'
	--OR LoadTypeCode = 'StageIncrementalWithHistoryUpdateLoad_LVD'
	--)
			...
			PRINT('Deploying Load Proc to StageArea for LoadConfig: ' + CONVERT(VARCHAR, @LoadConfigID))

			DECLARE @db sysname = 'DEV_StageArea'
			--DECLARE @db sysname = N''SELECT  DC.udf_get_DatabaseID_DatabasePurposeCode_DatabaseEnvironmentType('StageArea','dev'); --TODO: Make this dynamic with parameters 
			--SELECT	@db = d.DatabaseName
			--		--, dp.DatabasePurposeCode
			--		--, dp.DatabasePurposeName
			--		--, env.DetailTypeCode AS DatabaseEnvironmentCode
			--FROM	DC.[Database] d
			--	INNER JOIN [DC].tvf_get_DatabasesWithPurpose('StageArea') dp
			...
			BEGIN TRY
				...
				DECLARE @ProcName varchar(100) = (Select [DMOD].[udf_get_StageAreaProcName](@LoadConfigID))
				...
				BEGIN
					...
					PRINT('Deployement succeeded LoadConfig: ' + CONVERT(VARCHAR, @LoadConfigID))
				END
				ELSE
					BEGIN
						INSERT INTO #ErrorLog (LoadConfigID, Result, ErrorText)
						...
						PRINT('Generation succeeded, BUT DEPLOYMENT FAILED LoadConfig: ' + CONVERT(VARCHAR, @LoadConfigID))
					END
			...
			BEGIN CATCH
				...
				PRINT('Deployement FAILED LoadConfig: ' + CONVERT(VARCHAR, @LoadConfigID))
