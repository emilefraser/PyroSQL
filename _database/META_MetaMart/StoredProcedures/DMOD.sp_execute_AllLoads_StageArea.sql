...
/*
	...
	How? 
		...
		DECLARE @DatabaseName VARCHAR(150) = 'DEV_StageArea'
		
		EXEC DMOD.sp_execute_AllLoads_StageArea @Today, @IsInitialLoad, @IsTestLoad, @DatabaseName
*/

CREATE        PROCEDURE [DMOD].[sp_execute_AllLoads_StageArea]
...
--AND Target_DEName LIKE '%PIGMEATCARCASS%'
...
BEGIN  
			...
			IF OBJECT_ID(QUOTENAME(@Target_DatabaseName) + '.' + QUOTENAME(@Target_SchemaName) + '.' + QUOTENAME(@Target_TableName), N'U') IS NULL
			BEGIN
			...
			IF OBJECT_ID(QUOTENAME(@Target_DatabaseName) + '.' + QUOTENAME(@Target_SchemaName) + '.' + QUOTENAME(@Target_TableName), N'U') IS NOT NULL
			BEGIN

				-- Truncate Table and Corresponding Hist Tables 
				-- EF JUST @InitialLoad = 1, NOT Load=FullLoad as @InitialLoad is FullLoad is not neccessarily InitiaLoad
				IF (@IsInitialLoad = 1)
				BEGIN

					DECLARE @TruncateStatement nvarchar(max) = 'TRUNCATE TABLE ' + QUOTENAME(@Target_DatabaseName) + '.' + QUOTENAME(@Target_SchemaName) + '.' + QUOTENAME(@Target_TableName)
					...
					SET @TruncateStatement = 'TRUNCATE TABLE ' + QUOTENAME(@Target_DatabaseName) + '.' + QUOTENAME(@Target_SchemaName) + '.' + QUOTENAME(@Target_TableName + '_Hist')
					PRINT(@TruncateStatement)
					EXEC  @RC = sp_executesql @TruncateStatement
					IF(@RC = 0)
					BEGIN
				...
				BEGIN
						INSERT INTO #LoadErrorLog_DV
						VALUES
						(@LoadConfigID,@Target_DatabaseName,@Target_SchemaName , @Target_TableName,'ERROR! There is not a Load Procedure in StageArea for LoadConfigID: ')
				...
				BEGIN
					...
					BEGIN
						...
						PRINT(CHAR(9) + CHAR(9) +'ERROR! The following procedure failed at runtime ' + CONVERT(VARCHAR(MAX), QUOTENAME(@Target_DatabaseName) + '.' + QUOTENAME(@Target_SchemaName) + '.' + @ProcSQL))
					END
				END
			...
			BEGIN
				INSERT INTO #LoadErrorLog_DV
				VALUES
				(@LoadConfigID,@Target_DatabaseName,@Target_SchemaName , @Target_TableName,'ERROR! The following table does not exists in the StageArea: ' + CONVERT(VARCHAR(MAX),QUOTENAME( @Target_DatabaseName) + '.' + QUOTENAME(@Target_SchemaName) + '.' + QUOTENAME( @Target_TableName)))
				PRINT(CHAR(9) + CHAR(9) + 'ERROR! The following table does not exists in the StageArea: ' + CONVERT(VARCHAR(MAX), QUOTENAME(@Target_DatabaseName) + '.' +QUOTENAME( @Target_SchemaName) + '.' + QUOTENAME(@Target_TableName)))
			END
