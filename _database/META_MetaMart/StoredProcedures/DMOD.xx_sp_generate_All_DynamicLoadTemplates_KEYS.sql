...
/*
	...
	How? 
		...
		EXEC DMOD.sp_generate_All_DynamicLoadTemplates_KEYS @Author
*/

CREATE PROC  [DMOD].[xx_sp_generate_All_DynamicLoadTemplates_KEYS] 
				@Author VARCHAR(100) 
AS
...
SELECT lc.LoadConfigID, 
    ...
    WHERE de.DataEntityTypeCode = 'KEYS'
			...
			WHILE @@FETCH_STATUS = 0
				...
				SET @message = ('Generating KEYS Load Proc for LoadConfig = ' + CONVERT(VARCHAR, @LoadConfigID) + ' (' + @source_dataentityname + ' to ' + @target_dataentityname + ')')
				PRINT(@message)
				...
				IF(@RC = 0)
				BEGIN
					SET @message = ('KEYS Load Proc for SUCCESSFULLY GENERATED for LoadConfig: ' + CONVERT(VARCHAR, @LoadConfigID))
			...
			PRINT('Deploying KEYS Load Proc to StageArea for LoadConfig: ' + CONVERT(VARCHAR, @LoadConfigID))
