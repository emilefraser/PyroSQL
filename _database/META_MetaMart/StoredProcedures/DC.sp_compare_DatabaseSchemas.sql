...
--DECLARE	@TargetDatabaseID int = 4
---- @ReturnType =(1 Schema , 2 DataEntity , 3 Field , 4 FieldExtras (Max,Scale,Precision,DataType))
--DECLARE	@ReturnType int = 4



DROP TABLE IF EXISTS #SourceF
CREATE TABLE #SourceF
(SchemaID int,SchemaName varchar(100), DataEntityID int,DataEntityName varchar(100), FieldID int,FieldName varchar(100),DataType varchar(30),MaxLenth int, Precision int , Scale int)
insert into #SourceF
select distinct fd.schemaid,fd.schemaname , fd.DataEntityID,fd.DataEntityName,fd.FieldID,fd.FieldName,fd.DataType,fd.MaxLength,fd.Precision,fd.Scale
...
(SchemaID int,SchemaName varchar(100), DataEntityID int,DataEntityName varchar(100), FieldID int,FieldName varchar(100),DataType varchar(30),MaxLenth int, Precision int , Scale int)
insert into #TargetF
select distinct fd1.schemaid,fd1.schemaname , fd1.DataEntityID,fd1.DataEntityName,fd1.FieldID,fd1.FieldName,fd1.DataType,fd1.MaxLength,fd1.Precision,fd1.Scale
from DC.vw_rpt_DatabaseFieldDetail fd1 where DatabaseID = @TargetDatabaseID

IF @ReturnType = 1
BEGIN

SELECT DISTINCT s.SchemaName AS SourceDBSchemaName ,T.SchemaName AS TargetDBSchemaName FROM #SourceF S
...
SELECT DISTINCT S.SchemaName AS SourceSchemaName,s.DataEntityName AS SourceDBDataEntityName,t.SchemaName AS TargetSchemaName,T.DataEntityName AS TargetDBEntityName FROM #SourceF S
FULL OUTER JOIN #TargetF T ON
...
SELECT DISTINCT s.DataEntityName AS SourceDataEntityName,s.FieldName AS SourceFieldName,t.DataEntityName AS TargetDataEntityName ,T.FieldName AS TargetFieldName FROM #SourceF S
FULL OUTER JOIN #TargetF T ON
