...
CREATE VIEW [DC].[vw_RunCrawlerOnAzure]
AS 
 
 SELECT   1 AS DCDatabaseInstanceID --Change
        , 141 AS DatabaseID --Change
        , 'DataManager_DEV' AS DatabaseName --Change
        ...
        , tsize.usedMB usedMB
        ,NULL AS DatabaseSize-- No sys.master_files on Azure DB(SELECT total_size_mb = CAST(sum(size)*8. / 1024 AS decimal(8,2)) FROM sys.master_files WITH(NOWAIT) WHERE database_id = 0 and [type] = 0 GROUP BY database_id) AS DatabaseSize
		,null as IsActive
