...
--!~ LoadTypeInfo
/*
	Template Version No.:                       |   V 3.00
	Template last update date:                  |   2019-09-20T05:13:04.2466667
	Template load Type code:                    |   DataVaultFullLoad_HUB
	Template load Type description:             |   Full Load of Hub Table in DataVault from Standard StageArea Table
	Template Author:                            |   Emile Fraser
	Stored Proc Create Date:                    |   2020-05-20T23:17:43.060
*/
-- End of LoadTypeInfo ~!
...
/*
	...
	EXEC [raw].[sp_loadhub_AGE_BusinessUnit] @Today, @IsTest
*/
CREATE   PROCEDURE [raw].[sp_loadhub_AGE_BusinessUnit]
    @Today DATETIME2(7)
,	@IsTest BIT
AS
...
BEGIN TRY 
	...
	--!~ LoadConfigID
			, @LoadConfigID int = 757
-- End of LoadConfigID ~!
				, @StartDate DATETIME2(7) = GETDATE()
				, @DataEntityName VARCHAR(100) = '[vw_Subsidiary]'
				, @SourceRowCount INT
				, @SourceSizeBytes INT
	...
	SET @sql = (SELECT [DataManager_Local].[ETL].[udf_get_TableSpaceAndRows]('[DEV_DataVault]', PARSENAME('[raw]',1), PARSENAME('[HUB_BusinessUnit]',1)))
...
/*User Input*/	,@StepDescription = 'Gets the PreLoad Taget Row Count and PreLoad Target Size [raw].[HUB_BusinessUnit]'
/*User Input*/	,@AffectedDatabaseName = '[DEV_DataVault]'
/*User Input*/	,@AffectedSchemaName = '[raw]'
/*User Input*/	,@AffectedDataEntityName = '[HUB_BusinessUnit]'
/*User Input*/	,@ActionPerformed = 'SELECT'
	...
	INSERT INTO [raw].[HUB_BusinessUnit]
	   (
			[HK_BusinessUnit]															
	   ,	[LoadDT]															
	   ,	[RecSrcDataEntityID]												
	...
	SELECT
			[HK_BusinessUnit] = stagetable.[BKHash],										
			[LoadDT] = stagetable.[LoadDT],											
			[RecSrcDataEntityID] = stagetable.[RecSrcDataEntityID], 				
--!~ Field List with no alias - ODS		 [AGE_Business_Unit__c],		 [ENTITY]-- End of Field List with no alias - ODS ~!
	FROM 
			[DEV_StageArea].[AGE].[dbo_BusinessUnit_AGE_KEYS] stagetable						
	WHERE NOT EXISTS (
						SELECT 
							1
						FROM 
							[raw].[HUB_BusinessUnit] AS dvtable						
						WHERE 
							dvtable.[HK_BusinessUnit] = stagetable.[BKHash]			
					  )
	ORDER BY 1
	...
	EXEC [DataManager_Local].[ETL].[sp_insert_ExecutionLogSteps]
					 @ExecutionLogID = @ExecutionLogID
	/*User Input*/	,@StepDescription = 'Gets the PostLoad Taget Row Count and PoastLoad Target Size [raw].[HUB_BusinessUnit]'
	/*User Input*/	,@AffectedDatabaseName = '[DEV_DataVault]'
	/*User Input*/	,@AffectedSchemaName = '[raw]'
	/*User Input*/	,@AffectedDataEntityName = '[HUB_BusinessUnit]'
	/*User Input*/	,@ActionPerformed = 'SELECT'
