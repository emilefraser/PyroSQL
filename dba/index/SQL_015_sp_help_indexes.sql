...
CREATE PROCEDURE [dbo].[sp_help_indexes](@TableName VARCHAR(255) = NULL)
AS
BEGIN
SET NOCOUNT ON;
...
--##############################################################################
  ...
  INSERT INTO @Results
    ...
    FROM sys.objects OBJS
      INNER JOIN sys.partitions sp ON OBJS.object_id = sp.object_id 
...
SELECT Row_number()
	   ...
	   'DROP INDEX '+ QUOTENAME(r.index_name) 
         + ' ON '
         + QUOTENAME(r.schema_name)
         + '.' 
         + QUOTENAME(r.object_name) AS DropCmd,
       'IF NOT EXISTS(SELECT * FROM sys.indexes WHERE name = ''' 
         ...
         +  r.index_where_statement  
         + ' WITH ( DATA_COMPRESSION = ' + r.[data_compression_desc] + ' )'  As cmd
