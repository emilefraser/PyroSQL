USE master;
GO
IF OBJECT_ID('[dbo].[sp_help_waits]') IS NOT NULL 
DROP  PROCEDURE [dbo].[sp_help_waits] 
GO
--#################################################################################################
-- Real World DBA Toolkit version 4.94 Lowell Izaguirre lowell@stormrage.com
--#################################################################################################
CREATE PROCEDURE [dbo].[sp_help_waits]
AS

--#################################################################################################
-- 2016-11-29 14:23:49.261 SFCCN\lizaguirre 
-- Context: SUNPRDEDW01 msdb
--#################################################################################################
WITH [Waits]
     AS (SELECT
           [wait_type],
         ...
         WHERE  [wait_type] NOT IN ( N'BROKER_EVENTHANDLER', N'BROKER_RECEIVE_WAITFOR', N'BROKER_TASK_STOP', N'BROKER_TO_FLUSH',
                                     ...
                                     N'LOGMGR_QUEUE', N'ONDEMAND_TASK_QUEUE', N'PWAIT_ALL_COMPONENTS_INITIALIZED', N'QDS_PERSIST_TASK_MAIN_LOOP_SLEEP',
                                     ...
                                     N'SQLTRACE_BUFFER_FLUSH', N'SQLTRACE_INCREMENTAL_FLUSH_SLEEP', N'SQLTRACE_WAIT_ENTRIES', N'WAIT_FOR_RESULTS',
...
GROUP      BY
  [W1].[RowNum]
HAVING
  Sum ([W2].[Percentage]) - Max ([W1].[Percentage]) < 95; -- percentage thresholdGO
