/****** Object:  StoredProcedure [DMOD].[sp_ddl_Create_CRUD_Header]    Script Date: 2020/05/26 4:17:15 PM ******/
SET ANSI_NULLS ON
GO
...
======================================================================================================================== */
CREATE OR ALTER PROCEDURE [MASTER].[sp_create_CrudProcedure](
    @TargetDatabaseName		SYSNAME 
,	@TargetSchemaName		SYSNAME 
,	@TargetTableName		SYSNAME
,	@CrudType			NVARCHAR(100) -- Either Insert, InsertUpdate, InsertDelete, InsertUpdateDelete, DeleteUpdate
)
AS
...
BEGIN
	...
	DECLARE @sql_crlf2		NVARCHAR(4) = REPLICATE(@sql_crlf, 2)
	DECLARE @sql_tab		NVARCHAR(1) = CHAR(9)
	DECLARE @sql_continue	BIT = 1		
	DECLARE @sql_debug		BIT
	DECLARE @sqlexecute		BIT
	...
	DECLARE @IsCreate BIT = IIF(CHARINDEX(@CrudType, 'Create') > 0, 1, 0)
	DECLARE @IsInsert BIT = IIF(CHARINDEX(@CrudType, 'Insert') > 0, 1, 0)
	DECLARE @IsUpdate BIT = IIF(CHARINDEX(@CrudType, 'Update') > 0, 1, 0)
	DECLARE @IsDelete BIT = IIF(CHARINDEX(@CrudType, 'Delete') > 0, 1, 0)
