...
CREATE   PROCEDURE [PERFMASTER].[sp_Execute_TestOrchestrator]
	@TestClass				SYSNAME,
...
BEGIN
	...
	IF NOT EXISTS (
		SELECT 1 WHERE OBJECT_ID('PERLOG.RunTimeLog') IS NOT NULL
		AND OBJECT_ID('PERLOG.MetricsLog') IS NOT NULL
	)
	BEGIN
	SET @sql_message = 'LOG TABLES PERFTEST.RunTimeLog and PERFTEST.MetricsLog DOES NOT EXITS'
		RAISERROR(@sql_message, 1 , 1) WITH NOWAIT
	END
	...
	BEGIN
			...
			RAISERROR(@sql_message, 0 , 1) WITH NOWAIT
				...
				BEGIN TRANSACTION
					...
					EXEC [PERFMASTER].[sp_Update_MetricsLog] @LogID
