USE master;
GO
IF OBJECT_ID('[dbo].[sp_help_primarykeys]') IS NOT NULL 
DROP  PROCEDURE [dbo].[sp_help_primarykeys] 
GO
--#################################################################################################
-- Real World DBA Toolkit version 4.94 Lowell Izaguirre lowell@stormrage.com
--#################################################################################################
--sp_help_indexesWhere edlogdetail
CREATE PROCEDURE [dbo].[sp_help_primarykeys](@TableName VARCHAR(255) = NULL)
AS
BEGIN
SET NOCOUNT ON;
...
SELECT Row_number()
         OVER(
           ...
           quotename(r.schema_name) + '.' + quotename(r.object_name) As QualifiedObjectName,
       r.schema_id ,
       r.schema_name ,
       r.object_id ,
       r.object_name ,
...
AND (r.is_primary_key = 1 OR r.is_unique_constraint = 1)
...
EXECUTE sys.sp_MS_marksystemobject 'sp_help_primarykeys'
GRANT EXECUTE ON dbo.sp_help_primarykeys TO PUBLIC;
--#################################################################################################